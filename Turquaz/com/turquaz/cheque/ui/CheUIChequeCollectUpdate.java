...
public class CheUIChequeCollectUpdate extends org.eclipse.swt.widgets.Dialog
{
	...
	public CheUIChequeCollectUpdate(Shell parent, int style, TurqChequeRoll chequeRoll)
	{
		super(parent, style);
	...
	private void toolUpdateWidgetSelected(SelectionEvent evt)
	{
		try
		{
			if (compChequeRoll.verifyFields())
			{
				...
				CheBLUpdateChequeRoll.updateChequeCollectRoll(chequeRoll, compChequeRoll.getCashCardPcker().getTurqCashCard(),
						compChequeRoll.getTxtRollNo().getText().trim(), compChequeRoll.getDatePicker1().getDate(), chequeList);
