...
/**
 * @author  Onsel
 * @version  $Id: CheUIChequeInPayrollUpdate.java,v 1.16 2005/03/19 12:48:30 cemdayanik Exp $
 */
		...
		try
		{
			if (compChequeRoll.verifyFields())
			{
				...
				CheBLUpdateChequeRoll.updateChequeRollIn(chequeRoll, compChequeRoll.getAccountPicker().getTurqAccountingAccount(),
						(TurqCurrentCard) compChequeRoll.getCurrentPicker().getData(), null, compChequeRoll.getTxtRollNo().getText()
								.trim(), compChequeRoll.getDatePicker1().getDate(), chequeList, EngBLCommon.CHEQUE_TRANS_IN
								.intValue(), compChequeRoll.getBtnSumTotals().getSelection(), EngBLCommon
