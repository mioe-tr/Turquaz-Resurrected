...
public class MenuManager {
	...
	static void createEditMenu(final Menu menuMain) {
		...
		mitEdit.setMenu(menuEdit);
		{
			...
			menuItemPreferences
					.setText(Messages.getString("EngUIMainFrame.55")); //$NON-NLS-1$
		...
		mitView.setText(Messages.getString("EngUIMainFrame.14")); //$NON-NLS-1$
		{
			...
			mitView.setMenu(menuView);
			{
				...
				mitModules.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent evt) {
						...
						if (EngUIMainFrame
								.checkTabMenu(EngUIMainFrame.compModulesTab) == -1) {
							CTabItem item = new CTabItem(
									EngUIMainFrame.tabfldMenu, SWT.NULL);
			...
			}
			{
				...
				mitFavorites.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent evt) {
						if (EngUIMainFrame
								.checkTabMenu(EngUIMainFrame.compFavoritesTab) == -1) {
							CTabItem item = new CTabItem(
									EngUIMainFrame.tabfldMenu, SWT.NULL);
			...
			}
			{
				...
				mitHistory.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent evt) {
						if (EngUIMainFrame
								.checkTabMenu(EngUIMainFrame.compHistoryTab) == -1) {
							CTabItem item = new CTabItem(
									EngUIMainFrame.tabfldMenu, SWT.NULL);
			...
			{
				final MenuItem menuItemModulBar = new MenuItem(menuView,
						SWT.CHECK);
				menuItemModulBar.setText(Messages
						.getString("EngUIMainFrame.30")); //$NON-NLS-1$
				...
				menuItemModulBar.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent evt) {
						if (!menuItemModulBar.getSelection()) {
							EngUIMainFrame.sashMainVertical
									.setMaximizedControl(EngUIMainFrame.sashMainHorizontal);
						} else {
							EngUIMainFrame.sashMainVertical
									.setMaximizedControl(null);
						}
		...
		mitGoToRightTab.setImage(SWTResourceManager
				.getImage("/icons/forward.gif")); //$NON-NLS-1$
		mitGoToRightTab.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent evt) {
				if (EngUIMainFrame.tabfldMain.getItemCount() == 0) {
					return;
				} else if (EngUIMainFrame.tabfldMain.getItemCount() == EngUIMainFrame.tabfldMain
						.getSelectionIndex() + 1) {
					return;
				} else {
					EngUIMainFrame.tabfldMain
							.setSelection(EngUIMainFrame.tabfldMain
									.getSelectionIndex() + 1);
		...
		mitGoToLeftTab.setImage(SWTResourceManager
				.getImage("/icons/backward.gif")); //$NON-NLS-1$
