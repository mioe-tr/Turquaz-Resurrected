 ...
 * @version $Id: EngBLCommon.java,v 1.63 2005/03/16 09:53:38 cemdayanik Exp $
 */
...
public class EngBLCommon {
	...
	public final static String COMMON_BUY_STRING = Messages
			.getString("EngBLCommon.0"); //$NON-NLS-1$

	public final static String COMMON_SELL_STRING = Messages
			.getString("EngBLCommon.1"); //$NON-NLS-1$

	public final static String COMMON_ALL_STRING = Messages
			.getString("EngBLCommon.2"); //$NON-NLS-1$

	public final static int COMMON_DEPT = 0;

	public final static int COMMON_CREDIT = 1;

	public final static String COMMON_DEPT_STRING = Messages
			.getString("EngBLCommon.3"); //$NON-NLS-1$

	public final static String COMMON_CREDIT_STRING = Messages
			.getString("EngBLCommon.4"); //$NON-NLS-1$
	...
	public final static boolean CURRENT_TRANS_CREDIT = true; // cari alacak
															 // hareketi

	public final static boolean CURRENT_TRANS_DEBIT = false; // cari borc
															 // hareketi
	...
	public final static Integer CHEQUE_TRANS_IN =new Integer(0); //Cek Giris Bordrosu
	public final static String CHEQUE_TRANS_IN_STRING = Messages.getString("EngBLCommon.5"); //$NON-NLS-1$
	

	public final static Integer CHEQUE_TRANS_OUT_CURRENT = new Integer(1); //Cek Cari Cikis Bordrosu
	public final static String CHEQUE_TRANS_OUT_CURRENT_STRING = Messages.getString("EngBLCommon.6"); //$NON-NLS-1$

	public final static Integer CHEQUE_TRANS_OUT_BANK =  new Integer(2); //Cek bank Cikis bordrosu
	public final static String CHEQUE_TRANS_OUT_BANK_STRING = Messages.getString("EngBLCommon.7"); //$NON-NLS-1$

	public final static Integer CHEQUE_TRANS_COLLECT_FROM_BANK =  new Integer(3); //Bankadan Cek
	public final static String CHEQUE_TRANS_COLLECT_FROM_BANK_STRING = Messages.getString("EngBLCommon.8");															// Tahsilati //$NON-NLS-1$

	public final static Integer CHEQUE_TRANS_COLLECT_FROM_CURRENT =  new Integer(4); //Elden cek tahsilati
    public final static String CHEQUE_TRANS_COLLECT_FROM_CURRENT_STRING = Messages.getString("EngBLCommon.9"); //$NON-NLS-1$
	
	
	public final static Integer CHEQUE_TRANS_RETURN_FROM_BANK_TO_PORTFOY =  new Integer(5); //Bankadan Portfoye iade...
	...
	public static TurqCurrencyExchangeRate getCurrencyExchangeRate(
			TurqCurrency baseCurrency, TurqCurrency exchangeCurrency,
			Date exhangeDate) throws Exception {

		try {
			return EngDALCommon.getCurrencyExchangeRate(baseCurrency,
					exchangeCurrency, exhangeDate);
	...
	public static TurqCurrencyExchangeRate getBaseCurrencyExchangeRate() {
		try {
			if (baseCurrencyExchangeRate == null)
				baseCurrencyExchangeRate = AccBLTransactionSearch
	...
	public static void exportCurrentCardAccs() throws Exception {
		...
		try {
/*
			List ls = new CurDALCurrentCardSearch()
					.getCurrentCardsAndAccountingAccounts();
...
/*
			...
			for (int i = 0; i < ls.size(); i++) {
				...
				Object[] cashTranses = (Object[]) ls.get(i);
				TurqCashTransaction cashTrans = blSearch
						.initializeCashTransaction((Integer) cashTranses[0]);
				
				if (cashTrans.getTurqCashTransactionType().getId().intValue() == EngBLCommon.CASH_CURRENT_COLLECT
						|| cashTrans.getTurqCashTransactionType().getId()
								.intValue() == EngBLCommon.CASH_CURRENT_PAYMENT) {

					BigDecimal totalAmount = new BigDecimal(0);

					
					Iterator it = cashTrans.getTurqCashTransactionRows()
							.iterator();
					...
					if (it.hasNext()) {
						...
						TurqCashTransactionRow row = (TurqCashTransactionRow) it
								.next();
						cashCard = row.getTurqCashCard();

						if (row.getDeptAmount().compareTo(new BigDecimal(0)) == 1) {

							totalAmount=
									row.getDeptAmountInForeignCurrency();

						} else {
							totalAmount = row.getCreditAmountInForeignCurrency();
						}
