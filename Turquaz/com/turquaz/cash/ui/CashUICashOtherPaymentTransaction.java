...
/**
 * @author  Onsel
 * @version  $Id: CashUICashOtherPaymentTransaction.java,v 1.9 2005/03/30 16:57:43 onsel Exp $
 */
...
public class CashUICashOtherPaymentTransaction extends org.eclipse.swt.widgets.Composite implements SecureComposite
{
	...
	private void initGUI()
	{
		try
		{
			...
			lblCurrency.setText(Messages.getString("CashUICashOtherPaymentTransaction.1")); //$NON-NLS-1$
			//END << lblCurrency
		...
		try
		{
			if (verifyFields())
			{
				CashBLCashTransactionAdd.saveOtherTransaction((TurqCashCard) txtCashCard.getData(),
						(TurqAccountingAccount) txtAccountingAccount.getData(), EngBLCommon.CASH_OTHER_PAYMENT, null,
						curTextTotalAmount.getBigDecimalValue(), datePicker.getDate(), txtDefinition.getText(), txtDocumentNo
								.getText().trim(), exchangeRate);
	...
	public boolean verifyFields()
	{
		try
		{
			...
			{
				msg.setMessage(Messages.getString("CashUICashOtherPaymentTransaction.2")); //$NON-NLS-1$
				msg.open();
			...
			if (baseCurrency.getId().intValue() != exchangeCurrency.getId().intValue())
			{
				...
				{
					msg.setMessage(Messages.getString("CashUICashOtherPaymentTransaction.3")); //$NON-NLS-1$
					msg.open();
					return false;
				}
			}
