 ...
 * @version  $Id: AccUITransactionCollect.java,v 1.35 2005/01/04 11:13:55 onsel Exp $
 */
...
public class AccUITransactionCollect extends Composite implements
		SecureComposite {
	...
	private final String ACCOUNT_CODE = Messages
			.getString("AccUITransactionCollect.4"); //$NON-NLS-1$
	...
	private final String CREDIT = Messages
	...
	public void initGUI() {
		try {
			...
			}
			{
				...
				lblDate
						.setText(Messages
								.getString("AccUITransactionCollect.3")); //$NON-NLS-1$
...
//				enter empty table rows.
		...
		cursor.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				...
				if (e.keyCode == SWT.INSERT) {
					...
					cursor.setSelection(
							tableTransactionRows.getItemCount() - 1, 0);
	...
	public void save() {
		...
		if (verifyFields()) {
			...
			try {
				...
				msg
						.setMessage(Messages
								.getString("AccUITransactionCollect.19")); //$NON-NLS-1$
				msg.open();
	...
	public static void showGUI() {
		try {
			...
			AccUITransactionCollect inst = new AccUITransactionCollect(shell,
					SWT.NULL);
