/*
 * Created on Mar 30, 2005
 *
 * 
 * Window - Preferences - Java - Code Style - Code Templates
 */
package com.turquaz.engine;

/**
 * @author onsel
 *
 * 
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class EngKeys
{	
	
	public final static String VALUE="value";
	public final static String BILL_TEMPLATES ="billTemplates";
	public final static String AMOUNT_IN="amountIn";
	public final static String AMOUNT_OUT="amountOut";
	public final static String CONSTANT="constant";
	public final static String CREDIT_AMOUNT ="creditAmount";
	public final static String CURRENCIES="currencies";
	public final static String CURRENCY="currency";
	public final static String CURRENCY_ABBR="currencyAbbr";
	public final static String CURRENCY_ID="currencyId";
	public final static String DATABASE_VERSION="databaseVersion";
	public final static String DATE="date";
	public final static String DATE_END="dateEnd";
	public final static String DATE_START="dateStart";
	public final static String DEFAULT="default";
	public final static String DEFINITION="definition";
	public final static String DEPT_AMOUNT ="deptAmount";
	public final static String DESCRIPTION="description";
	public final static String DOCUMENT_NO="documentNo";
	public final static String DOCUMENT_NO_END="documentNoEnd";	
	public final static String DOCUMENT_NO_START="documentNoStart";
	public final static String DUE_DATE_END="DueDateEnd";
	public final static String DUE_DATE_START="DueDateStart";
	public final static String ENG_SEQ="engSeq";
	public final static String ENG_SEQ_ID="engSeqId";
	public final static String EXCHANGE_CURRENCY="exchangeCurrency";
	public final static String EXCHANGE_CURRENCY_ABBR = "exchangeCurrencyAbbr";
	public final static String EXCHANGE_CURRENCY_ID = "exchangeCurrencyId";
	public final static String EXCHANGE_DATE="exchangeDate";
	public final static String EXCHANGE_RATE = "exchangeRate";
	public final static String EXCHANGE_RATE_ID = "exchangeRateId";
	public final static String EXCHANGE_RATIO="exchangeRatio";
	public final static String IS_CREDIT="isCredit";
	public final static String MAX_VALUE="maxValue";
	public final static String MIN_VALUE="minValue";
	public final static String MIN_VALUE_TRANSACTION="minValueTransaction";
	public final static String MIN_VALUE_BALANCE="minValueBalance";
	public final static String OBJECT="object";
	public final static String PASSWORD ="password";
	public final static String TOTAL_AMOUNT="totalAmount";
	public final static String TRANS_DATE="transDate";	
	public final static String TRANS_ID="transId";
	public final static String TYPE="type";
	public final static String TYPE_ID="typeId";
	public final static String TYPE_NAME="typeName";
	public final static String TYPES="types";
	public final static String USER ="user";
	public final static String RETURN_VALUE ="returnValue";
	public final static String MODULE_COMPONENTS="moduleComponents";
	public final static String GROUP_PERMISSIONS="groupPermissions";
	public final static String USER_PERMISSIONS="userPermissions";
	public final static String REPORT="report";
	public final static String MENUS="menus";
	public final static String MENU_NAME="menuName";
	public final static String MENU_TYPE="menuType";
	public final static String MODULE_COMPONENT_CLASS="moduleComponentClass";
	public final static String PERMISSION_MAP ="permissionMap";
	public final static String PRICE="price";
}
