...
/**
 * @author Onsel
 * @version $Id: CheBLUpdateChequeRoll.java,v 1.1 2007/01/22 16:36:34 huseyiner Exp $
 */
...
public class CheBLUpdateChequeRoll
{
	...
	public static HashBag getChequeRollInfo(HashMap argMap)throws Exception
	{
		...
		while (it.hasNext())
		{
			...
			rollInfo.put(CheKeys.CHE_CHEQUE_LIST,i,CheKeys.CHE_ALREADY_IN_ROLL,new Boolean(true));
		...
		if(chequeRoll.getTurqCurrentCard().getId().intValue()!=-1)
		{
			
			rollInfo.put(CurKeys.CUR_CURRENT_NAME,chequeRoll.getTurqCurrentCard().getCardsName());
			rollInfo.put(CurKeys.CUR_CURRENT_CODE,chequeRoll.getTurqCurrentCard().getCardsCurrentCode());
		}
		if(chequeRoll.getTurqBanksCard().getId().intValue()!=-1)
		{
			
			rollInfo.put(BankKeys.BANK_CODE,chequeRoll.getTurqBanksCard().getBankCode());
			
		}
		if(chequeRoll.getTurqChequeRollAccountingAccount()!=null)
		{
			rollInfo.put(AccKeys.ACC_ACCOUNT_CODE,chequeRoll.getTurqChequeRollAccountingAccount().getTurqAccountingAccount().getAccountCode());
	...
	public static void updateChequeCollectRoll(HashMap argMap) throws Exception
	{
		...
		String rollNo = (String)argMap.get(EngKeys.DOCUMENT_NO);
		 ...
		 List chequeList = (List)argMap.get(CheKeys.CHE_CHEQUE_LIST);
			...
			removeAllChequesFromRoll(chequeRoll);
			registerChequesToRoll(chequeRoll,chequeList);
            
            SessionInfo info = MyThreadLocal.getSessionInfo();            
            String username =(String)info.get("username");
            
			chequeRoll.setUpdatedBy(username);
			...
			CashBLCashTransactionAdd.saveCashTransaction(cashCard, chequeRoll.getTurqEngineSequence(), EngBLCommon.CASH_CHEQUE_COLLECT,
					...
					totals, account, EngDALCommon.getBaseCurrencyExchangeRate()); //$NON-NLS-1$
			
			CheBLSaveChequeTransaction.saveRollAccountingTransactions(cashCard.getTurqAccountingAccount(), null, chequeRoll,
					chequeTotals, EngDALCommon.getBaseCurrencyExchangeRate(),
	...
	public static void updateChequeRollIn(HashMap argMap) throws Exception
	{
			...
			if(bankCardId!=null)
			{
			 ...
			 TurqCurrencyExchangeRate exchangeRate =(TurqCurrencyExchangeRate) EngDALCommon.getCurrencyExchangeRate(currencyId,rollDate);
			...
			else if (rollType == EngBLCommon.CHEQUE_TRANS_COLLECT_FROM_BANK.intValue())
			{
				...
				CheBLSaveChequeTransaction.saveRollAccountingTransactions(null, null, chequeRoll, null, EngDALCommon
						.getBaseCurrencyExchangeRate(),  CheServerLangKeys.ROLL_NO + chequeRoll.getChequeRollNo()); //$NON-NLS-1$
			}
			else if (rollType == EngBLCommon.CHEQUE_TRANS_RETURN_FROM_BANK_TO_PORTFOY.intValue())
			{
				CheBLSaveChequeTransaction
						.saveRollAccountingTransactions(rollAccount, null, chequeRoll, totalAmount, EngDALCommon
	...
	private static void registerChequesToRoll(TurqChequeRoll chequeRoll, List chequeList)throws Exception
	{
		...
		for (int i = 0; i < chequeList.size(); i++)
		{
			...
			chequeInRoll.setCreatedBy(username); //$NON-NLS-1$
			chequeInRoll.setUpdatedBy(username); //$NON-NLS-1$
		...
		EngDALSessionFactory.getSession().refresh(chequeRoll);
		
	}
	private static void removeAllChequesFromRoll(TurqChequeRoll chequeRoll)throws Exception
	{
	...
	public static HashBag getChequeStatus(HashMap argMap)throws Exception
	{
		Integer chequeId =(Integer)argMap.get(CheKeys.CHE_CHEQUE_ID);
		
		Integer chequeRollId = CheDALSearch.getChequeStatus(chequeId);
		
		HashBag returnBag = new HashBag();
		
		returnBag.put(CheKeys.CHE_CHEQUE_ROLL_ID,chequeRollId);
