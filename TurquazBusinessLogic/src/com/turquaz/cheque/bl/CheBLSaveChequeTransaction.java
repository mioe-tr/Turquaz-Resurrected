...
/**
 * @author Onsel
 * @version $Id: CheBLSaveChequeTransaction.java,v 1.1 2007/01/22 16:36:34 huseyiner Exp $
 */
...
public class CheBLSaveChequeTransaction
{
	...
	public static void saveChequeRoll(HashMap argMap) throws Exception
	{
		...
		TurqCurrencyExchangeRate exchangeRate = EngDALCommon.getCurrencyExchangeRate(EngDALCommon.getBaseCurrency().getId(),rollDate);
			...
			chequeRoll.setCreatedBy(username);
			...
			for (int i = 0; i < chequeList.size(); i++)
			{
				...
				chequeInRoll.setCreatedBy(username);
				chequeInRoll.setUpdatedBy(username);
	...
	private static TurqChequeCheque saveCheque(HashMap chequeInfo,Date rollDate)throws Exception
	{
		...
		Integer bankCardId =(Integer)chequeInfo.get(BankKeys.BANK_ID);		
	...
	public static void saveCollectOfOwnCheques(HashMap argMap)throws Exception
	{
		...
		for (int i = 0; i < chequeList.size(); i++)
		{
			...
			cheque =(TurqChequeCheque)EngDALSessionFactory.getSession().load(TurqChequeCheque.class,chequeId);
			...
			throws Exception
	{
		...
		else if (type == EngBLCommon.CHEQUE_TRANS_COLLECT_FROM_CURRENT.intValue())
		{
			...
			while (it.hasNext())
			{
				...
				{
					System.out.println(cheque.getChequesAmount());
					List ls = new ArrayList();
					ls.add(cheque.getChequesAmount());
					creditAccountsMap.put(creditAccount.getId(), ls);
				}
