...
public class CheBLUpdateCheque
{
	...
	private static void updateChequeRollTransactions(TurqChequeRoll chequeRoll) throws Exception
	{
			...
			updateCashTransactions(chequeRoll);
	
	}
	
	private static void updateCashTransactions(TurqChequeRoll chequeRoll)throws Exception
	{
		
		if(chequeRoll.getTurqChequeTransactionType().getId().intValue()==EngBLCommon.CHEQUE_TRANS_COLLECT_FROM_CURRENT.intValue())
		{
			
			TurqCashCard cashCard =null;
			Iterator it = chequeRoll.getTurqEngineSequence().getTurqCashTransactions().iterator();
			while (it.hasNext())
			{
				TurqCashTransaction cashTrans = (TurqCashTransaction)it.next();
				
				Iterator cashRowIt = cashTrans.getTurqCashTransactionRows().iterator();
				while(cashRowIt.hasNext())
				{
					
					TurqCashTransactionRow transRow = (TurqCashTransactionRow) cashRowIt.next();
					cashCard = transRow.getTurqCashCard();					
					
				}
				

				CashBLCashTransactionUpdate.deleteOnlyCashTransaction(cashTrans);
			}
			...
			if(cashCard!=null)
			{
				...
				Iterator rollIt = chequeRoll.getTurqChequeChequeInRolls().iterator();
				while(rollIt.hasNext())
				{
					TurqChequeChequeInRoll chequeInRoll = (TurqChequeChequeInRoll)rollIt.next();
				...
				if(amount.doubleValue()>0)
				{
					...
					CashBLCashTransactionAdd.saveCashTransaction(cashCard, chequeRoll.getTurqEngineSequence(), EngBLCommon.CASH_CHEQUE_COLLECT,
							chequeRoll.getChequeRollsDate(), CheServerLangKeys.ENTRY_ROLL, chequeRoll.getChequeRollNo(),totals, //$NON-NLS-1$
	...
	public static void updateBankTransactions(TurqChequeRoll chequeRoll) throws Exception
	{
			...
			EngDALSessionFactory.getSession().refresh(chequeRoll.getTurqEngineSequence());
