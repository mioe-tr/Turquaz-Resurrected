...
{
	public static List getInventoryLedger(Date date, String invCode, Integer warehouseId) throws Exception
	{
		try
		{
			...
			String query = "SELECT invCard.card_inventory_code,invCard.card_name, transin.totalamountin AS total_amount_in, transin.totalpricein AS total_price_in, transout.totalamountout AS total_amount_out "
                    ...
                    if (warehouseId != null)
                    {
                        query += " AND turq_inventory_transactions.inventory_warehouses_id =" + warehouseId ;
                    } 
            
					query+= " GROUP BY turq_inventory_transactions.inventory_cards_id) transin ON invCard.id = transin.inventory_cards_id "
					...
					query += " GROUP BY turq_inventory_transactions.inventory_cards_id) transout ON invCard.id = transout.inventory_cards_id"
