 ...
 * @version $Id: InvDALSearchTransaction.java,v 1.1 2007/01/22 16:36:34 huseyiner Exp $
 */
...
{
	public static List searchTransactions(Integer curCardId, Integer invCardId, Date startDate, Date endDate, int type , Integer warehouseId)
			throws Exception
	{
		try
		{
			...
			String query = "Select transaction.id,transaction.transactionsDate,transaction.amountIn,"
					+ " transaction.amountOut, transaction.totalPriceInForeignCurrency," + " transaction.turqInventoryCard.cardInventoryCode, "
					+ " transaction.turqInventoryCard.cardName, transaction.turqCurrencyExchangeRate.turqCurrencyByExchangeCurrencyId.currenciesAbbreviation from TurqInventoryTransaction as transaction"
            ...
            if (warehouseId != null)
            {
                query += " and transaction.turqInventoryWarehous.id =" + warehouseId; 
            }
