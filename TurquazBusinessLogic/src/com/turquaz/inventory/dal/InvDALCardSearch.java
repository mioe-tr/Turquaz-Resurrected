 ...
 * @version $Id: InvDALCardSearch.java,v 1.1 2007/01/22 16:36:34 huseyiner Exp $
 */
...
public class InvDALCardSearch
{
	...
	private static boolean isInventoryCardsDirty =true;
	private static boolean isInventoryPricesDirty=true;
	
	
	
		
	public static boolean isInventoryPricesDirty()
	{
		return isInventoryPricesDirty;
	}

	public static void setInventoryPricesDirty(boolean isInventoryPricesDirty)
	{
		InvDALCardSearch.isInventoryPricesDirty = isInventoryPricesDirty;
	}

	public static boolean isInventoryCardsDirty() {
		return isInventoryCardsDirty;
	}

	public static void setInventoryCardsDirty(boolean isInventoryCardsDirty) {
		InvDALCardSearch.isInventoryCardsDirty = isInventoryCardsDirty;
	}
			...
			throws Exception
	{
		try
		{
			...
			String query = "Select invCard, invView.totalAmountIn, invView.totalPriceIn," +
					" invView.totalAmountOut, invView.totalPriceOut from TurqViewInventoryTotal invView,"
	...
	public static String getGroupName(Integer groupId) throws Exception
	{
		Session session = EngDALSessionFactory.getSession();
		String query = "Select gr.groupsName from TurqInventoryGroup as gr"
				+ " where gr.id="+groupId;
		Query q = session.createQuery(query);
		List list = q.list();
		if (list.size() > 0)
	...
	public static List getInventoryCards() throws Exception
	{
		...
		{
			Session session = EngDALSessionFactory.getSession();
			String query = "Select invCard.cardInventoryCode,invCard.cardName,invCard.id from TurqInventoryCard as invCard"
					+ " order by invCard.cardInventoryCode";
			Query q = session.createQuery(query);
			q.setCacheable(true);
			if(isInventoryCardsDirty)
			{
				q.setCacheMode(CacheMode.REFRESH);
				isInventoryCardsDirty=false;
			}
			List list = q.list();
			return list;
		}
	...
	public static List getInventoryPrices() throws Exception
	{
		try
		{
			Session session = EngDALSessionFactory.getSession();
			String query = "Select invPrice.turqInventoryCard.id, invPrice.pricesType," +
					"invPrice.pricesAmount, invPrice.turqCurrency.id from TurqInventoryPrice invPrice";
			Query q = session.createQuery(query);
			q.setCacheable(true);
			if(isInventoryCardsDirty)
			{
