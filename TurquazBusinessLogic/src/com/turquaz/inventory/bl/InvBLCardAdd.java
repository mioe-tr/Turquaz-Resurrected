 ...
 * @version $Id: InvBLCardAdd.java,v 1.1 2007/01/22 16:36:35 huseyiner Exp $
 */
...
{	
	...
	public static void deleteInvUnit(HashMap argMap)throws Exception
	{
		Integer unitId = (Integer)argMap.get(InvKeys.INV_UNIT_ID);
		TurqInventoryUnit unit =(TurqInventoryUnit)EngDALSessionFactory.getSession().load(TurqInventoryUnit.class,unitId);
		EngDALCommon.deleteObject(unit);
			...
			throws Exception
	{
		try
		{
			...
			invAcc.setCreatedBy(username); //$NON-NLS-1$
			invAcc.setUpdatedBy(username); //$NON-NLS-1$
	...
	private static void registerInvCardGroup(TurqInventoryCard card, TurqInventoryGroup group) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			cardGroup.setCreatedBy(username); //$NON-NLS-1$
			cardGroup.setUpdatedBy(username); //$NON-NLS-1$
			Calendar cal = Calendar.getInstance();
	...
	private static void registerInvCardPrice(TurqInventoryCard card, boolean price_type,Integer currencyId,
			BigDecimal price) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			invPrice.setPricesAmount(price);
			invPrice.setTurqInventoryCard(card);
			invPrice.setTurqCurrency(currency);
			invPrice.setCreatedBy(username); //$NON-NLS-1$
			invPrice.setUpdatedBy(username); //$NON-NLS-1$
			Calendar cal = Calendar.getInstance();
			...
			throws Exception
	{
		...
		cardUnit.setCreatedBy(username); //$NON-NLS-1$
		cardUnit.setUpdatedBy(username); //$NON-NLS-1$
		Calendar cal = Calendar.getInstance();
			...
			boolean isSpecAmount) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			card.setCreatedBy(username); //$NON-NLS-1$
			card.setUpdatedBy(username); //$NON-NLS-1$
			Calendar cal = Calendar.getInstance();
	...
	public static void saveInitialTransaction(TurqInventoryCard invCard, TurqInventoryUnit unit) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			invTrans.setTransactionsDate(TurquazDateFormat.getFirstDayOfYear());
			...
			invTrans.setCreatedBy(username); //$NON-NLS-1$
			invTrans.setUpdatedBy(username); //$NON-NLS-1$
	...
	public static void saveInvCardPrices(TurqInventoryCard card, List invPrices) throws Exception
	{
		for (int k = 0; k < invPrices.size(); k++)
		{
			...
			registerInvCardPrice(card, ((Boolean) invPrice[0]).booleanValue(), (Integer) invPrice[1], (BigDecimal) invPrice[2]);
		}
	}
	...
	public static void saveInvGroup(HashMap argMap) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			invGroup.setCreatedBy(username); //$NON-NLS-1$
			invGroup.setUpdatedBy(username); //$NON-NLS-1$
			Calendar cal = Calendar.getInstance();
	...
	public static void saveUnit(HashMap argMap) throws Exception
	{
	    ...
	    String username =(String)info.get("username");
			...
			//invGroup.setGroupsDescription(groupDescription);
			invUnit.setCreatedBy(username); //$NON-NLS-1$
