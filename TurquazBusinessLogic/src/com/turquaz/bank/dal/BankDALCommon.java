...
/**
 * @author Onsel
 * @version $Id: BankDALCommon.java,v 1.1 2007/01/22 16:36:35 huseyiner Exp $
 */
...
public class BankDALCommon
{
	...
	public static List getTransactions(Integer bankCardId, Date startDate, Date endDate) throws Exception
	{
		try
		{
			Session session = EngDALSessionFactory.getSession();
			...
			String query = "SELECT bankTrans.transaction_bill_date,"
					...
					+ bankCardId.intValue() + " and bankTrans.transaction_bill_date >= '" + frmt.format(startDate) + "' and "
	...
	//Devreden Toplam
	public static List getDeferredTotal(Integer bankId, Date endDate) throws Exception
	{
		try
		{
			Session session = EngDALSessionFactory.getSession();
			String query = "Select sum(row.deptAmount),sum(row.creditAmount)"
					...
					+ " and row.turqBanksCard.id = "+bankId+" group by row.turqBanksCard";
			Query q = session.createQuery(query);
			q.setParameter("endDate", endDate);
			List ls = q.list();
