...
/**
 * @author Onsel
 * @version $Id: BankBLTransactionAdd.java,v 1.1 2007/01/22 16:36:34 huseyiner Exp $
 */
...
public class BankBLTransactionAdd
{
			...
			throws Exception
	{
		try
		{
			...
			bankTransBill.setCreatedBy(username); 
			bankTransBill.setUpdatedBy(username); 
			...
			transRowCredit.setCreatedBy(username); 
			transRowCredit.setUpdatedBy(username);
			...
			transRowDebit.setCreatedBy(username); 
			transRowDebit.setUpdatedBy(username); 
			...
			TurqAccountingAccount creditAccount = BankDALBankCardSearch.getBankAccountingAccount(bankCardWithDept,
					EngBLCommon.BANK_ACC_TYPE_GENERAL);
			TurqAccountingAccount deptAccount = BankDALBankCardSearch.getBankAccountingAccount(bankCardWithCredit,
					EngBLCommon.BANK_ACC_TYPE_GENERAL);
	...
	public static void saveInitialBankTransaction(TurqBanksCard bankCard) throws Exception
	{
			...
			transRow.setCreatedBy(username); //$NON-NLS-1$
			transRow.setUpdatedBy(username); //$NON-NLS-1$
			...
			String definition, String docNo, TurqCurrencyExchangeRate exRate) throws Exception
	{
		try
		{
			...
			bankTransBill.setCreatedBy(username); //$NON-NLS-1$
			bankTransBill.setUpdatedBy(username); //$NON-NLS-1$
			bankTransBill.setLastModified(Calendar.getInstance().getTime());
			bankTransBill.setCreationDate(Calendar.getInstance().getTime());
			/*
			 * Transaction Rows
