 ...
 * kimlik doğrulama kullanır; e-Arşiv gönderimi, durum sorgu ve iptal sağlar.
 *
 * NOT: Uç-nokta yolları ve JSON alan adları Nilvera geliştirici
 * dokümantasyonuna (developer.nilvera.com) göre canlıya geçişte teyit
 * edilmelidir; bu sınıf sağlayıcı arayüzünün referans uygulamasıdır ve
 * test ortamına (apitest.nilvera.com) karşı denenecek şekilde yazılmıştır.
 */
public class NilveraProvider implements EInvoiceProvider {
    ...
    public boolean supports(EInvoiceType type) {
        // Nilvera e-Arşiv, e-Fatura ve e-İrsaliye'yi destekler.
        return true;
    }
    ...
    public EInvoiceResult submit(EInvoice doc, ProviderCredentials creds) throws EInvoiceException {
        if (doc.getType() != EInvoiceType.EARSIV) {
            throw new EInvoiceException("Bu sürümde Nilvera adaptörü yalnızca e-Arşiv gönderimini uygular: "
                    + doc.getType());
        }
        ...
        try {
            ...
            throw new EInvoiceException("Nilvera e-Arşiv gönderiminde ağ hatası: " + e.getMessage(), e);
        }
    }
    ...
    public EInvoiceResult queryStatus(String ettn, ProviderCredentials creds) throws EInvoiceException {
        try {
            ...
            throw new EInvoiceException("Nilvera durum sorgusunda ağ hatası: " + e.getMessage(), e);
        }
    }
    ...
    public EInvoiceResult cancel(String ettn, String reason, ProviderCredentials creds)
        ...
        try {
            ...
            throw new EInvoiceException("Nilvera iptalde ağ hatası: " + e.getMessage(), e);
        }
    }
    ...
    private EInvoiceResult interpret(HttpJson.Response resp, String op) {
        ...
        EInvoiceResult r = EInvoiceResult.fail("Nilvera " + op + " başarısız (HTTP " + resp.code + ")");
        r.setRawResponse(resp.body);
        return r;
    }

    /**
     * e-Arşiv belgesini Nilvera "model" JSON gövdesine çevirir. Alan adları
     * Nilvera EArchiveModel şemasına yakın tutulmuştur; canlıda doküman ile
