...
/**
 * "Kes" iþ kuralý: bir Turquaz faturasýný seçili entegratöre gönderip
 * kesinleþtirir ve sonucu {@link TurqEInvoiceStatus} kaydýna yazar.
 *
 * <p>Akýþ: yükle -> mükerrer kontrolü -> yönlendir (entegratör + seri) -> maple
 * -> gönder -> durumu sakla. Fatura otomatik kesilmez; bu metot kullanýcýnýn
 * UI'daki "e-Arþiv Kes" aksiyonuyla tetiklenir.
 */
public class EinvoiceBLIssue {
    ...
    /**
     * Verilen faturayý keser.
     *
     * @param transactionId {@code TurqCurrentTransaction} id'si
     * @param type          belge tipi (þimdilik EARSIV)
     * @param settings      entegratör profilleri + yönlendirme + satýcý kimliði
     * @param currentUser   iþlemi yapan kullanýcý (audit alanlarý için)
     */
    public static EInvoiceResult issue(Integer transactionId, EInvoiceType type,
        ...
        try {
            ...
            if (header == null) {
                throw new EInvoiceException("Fatura bulunamadý: id=" + transactionId);
            }

            TurqEInvoiceStatus status = EinvoiceDAL.findStatus(transactionId);
            if (status != null && isFinal(status.getStatus())) {
                throw new EInvoiceException("Bu fatura zaten kesilmiþ (durum: "
            ...
            throw new EInvoiceException("e-belge kesme sýrasýnda beklenmeyen hata: " + e.getMessage(), e);
        }
    }
