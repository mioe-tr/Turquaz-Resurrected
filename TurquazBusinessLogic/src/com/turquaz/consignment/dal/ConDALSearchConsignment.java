...
public class ConDALSearchConsignment
{
	...
	public static List chooseConsignments(TurqCurrentCard curCard, Date startDate, Date endDate, Integer currencyId) throws Exception
	{
		try
		{
			Session session = EngDALSessionFactory.getSession();
			String query = "Select consignment.id," + " consignment.consignmentsDate, consignment.turqCurrentCard.cardsCurrentCode,"
					...
					" and consignment.turqCurrencyExchangeRate.turqCurrencyByExchangeCurrencyId.id="+currencyId;
			if (curCard != null)
			{
				query += " and consignment.turqCurrentCard = :curCard"; //$NON-NLS-1$
			}
