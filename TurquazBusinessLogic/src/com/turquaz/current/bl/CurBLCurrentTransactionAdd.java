 ...
 * @version $Id: CurBLCurrentTransactionAdd.java,v 1.1 2007/01/22 16:36:33 huseyiner Exp $
 */
...
public class CurBLCurrentTransactionAdd
{
	 ...
	 */
	private static void prepareAccountingMaps(TurqCurrentCard curCard, boolean isCredit, BigDecimal amount,
			TurqAccountingAccount account, Map deptAccounts, Map creditAccounts) throws Exception
	{
		try
		{
			deptAccounts.clear();
			...
			TurqAccountingAccount currentAccountingAccount = CurBLCurrentCardSearch
					.getCurrentAccountingAccount(curCard, EngBLCommon.CURRENT_ACC_TYPE_GENERAL);
	 ...
	 */
	//DONE
	public static void saveCurrentCashTransaction(HashMap argMap) throws Exception
	{
		Integer curCardId = (Integer) argMap.get(CurKeys.CUR_CARD_ID);
		TurqCurrentCard curCard = null;
		if (curCardId != null)
		{
			curCard = (TurqCurrentCard) EngDALSessionFactory.getSession().load(TurqCurrentCard.class,
					curCardId);
		}
		...
		if (type.intValue() == 4)
		{
			...
			prepareAccountingMaps(curCard, isCredit.booleanValue(), amount, account, deptAccounts,
					creditAccounts);
			AccBLTransactionAdd.saveAccTransaction(transDate, documentNo, accTransactionType, 4, seq.getId(),
					transDefinition, exchangeRate, creditAccounts, deptAccounts, true);
			...
			curTrans.setTransactionsTotalDiscount(totalDiscount.multiply(exchangeRate.getExchangeRatio())
			...
			curTrans.setCreatedBy(username);
			curTrans.setUpdatedBy(username);
	...
	public static TurqCurrentTransaction saveCurrentTransaction(TurqCurrentCard curCard, Date transDate,
			String documentNo, boolean isCredit, BigDecimal amount, BigDecimal totalDiscount, int type,
			Integer seqDocNo, String definition, TurqCurrencyExchangeRate exchangeRate) throws Exception
	{
		try
		{
			...
			if (isCredit)
			{
				curTrans.setTransactionsTotalCredit(amount.multiply(exchangeRate.getExchangeRatio())
						.setScale(2, EngBLCommon.ROUNDING_METHOD));
			...
			else
			{
				...
				curTrans.setTransactionsTotalDept(amount.multiply(exchangeRate.getExchangeRatio()).setScale(
	...
	public static void saveCurrentTransferBetweenAccounts(HashMap argMap) throws Exception
	{
		try
		{
			...
			TurqCurrentCard creditCard = (TurqCurrentCard) EngDALSessionFactory.getSession().load(
					TurqCurrentCard.class, creditCardId);
			TurqCurrentCard debitCard = (TurqCurrentCard) EngDALSessionFactory.getSession().load(
					TurqCurrentCard.class, deptCardId);
			Date transDate = (Date) argMap.get(EngKeys.DATE);
			String documentNo = (String) argMap.get(EngKeys.DOCUMENT_NO);
			BigDecimal amount = (BigDecimal) argMap.get(CurKeys.CUR_TRANS_AMOUNT);
			...
			Integer currencyId = (Integer) argMap.get(EngKeys.CURRENCY_ID);
			TurqCurrencyExchangeRate exchangeRate = EngDALCommon.getCurrencyExchangeRate(currencyId,
					transDate);
			if (exchangeRate == null)
			{
				throw new TurquazException(EngLangCommonKeys.MSG_DEFINE_DAILY_EXCHANGE_RATE);
			...
			seq.setTurqModule(module);
			EngDALCommon.saveObject(seq);
			TurqCurrentTransaction curTrans1 = saveCurrentTransaction(creditCard, transDate, documentNo,
					false, amount, new BigDecimal(0), EngBLCommon.CURRENT_TRANS_BETWEEN_ACCOUNTS,
					seq.getId(), definition, exchangeRate);
			TurqCurrentTransaction curTrans2 = saveCurrentTransaction(debitCard, transDate, documentNo, true,
					amount, new BigDecimal(0), EngBLCommon.CURRENT_TRANS_BETWEEN_ACCOUNTS, seq.getId(),
	...
	public static HashBag saveMultipleOtherTransaction(HashMap argMap) throws Exception
	{
		...
		TurqCurrentTransaction curTrans = saveCurrentTransaction(curCard, transDate, documentNo, isCredit
				.booleanValue(), amount, new BigDecimal(0), type.intValue(), null, definition, exchangeRate);
		Map creditMap = new HashMap();
		Map deptMap = new HashMap();
		TurqAccountingAccount currentAccountingAccount = CurBLCurrentCardSearch.getCurrentAccountingAccount(
				curCard, EngBLCommon.CURRENT_ACC_TYPE_GENERAL);
		if (currentAccountingAccount != null)
		{
			if (isCredit.booleanValue())
			{
			...
			AccBLTransactionAdd.saveAccTransaction(transDate, documentNo,
					EngBLCommon.ACCOUNTING_TRANS_GENERAL, EngBLCommon.MODULE_CURRENT, curTrans
							.getTurqEngineSequence().getId(), definition, exchangeRate, transColumns);
	...
	public static void saveOtherCurrentTransaction(HashMap argMap) throws Exception
	{
		try
		{
			Integer curCardId = (Integer) argMap.get(CurKeys.CUR_CARD_ID);
			...
			if (accountId != null)
			{
				account = (TurqAccountingAccount) EngDALSessionFactory.getSession().load(
			...
			TurqCurrentTransaction curTrans = saveCurrentTransaction(curCard, transDate, documentNo, isCredit
					.booleanValue(), amount, totalDiscount, type.intValue(), seqDocNo, definition,
					exchangeRate);
			if (account == null)
			{
				return;
			...
			else
			{
				...
				String transDefinition = "Cari Borc/Alacak " + TurquazDateFormat.format(transDate) + " "
						+ documentNo;
				Map creditAccounts = new HashMap();
				Map deptAccounts = new HashMap();
				prepareAccountingMaps(curCard, isCredit.booleanValue(), amount, account, deptAccounts,
				...
				AccBLTransactionAdd.saveAccTransaction(transDate, documentNo,
						EngBLCommon.ACCOUNTING_TRANS_GENERAL, EngBLCommon.MODULE_CURRENT, curTrans
								.getTurqEngineSequence().getId(), transDefinition, exchangeRate,
