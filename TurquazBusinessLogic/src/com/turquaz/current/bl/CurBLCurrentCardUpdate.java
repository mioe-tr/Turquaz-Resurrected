 ...
 * @version $Id: CurBLCurrentCardUpdate.java,v 1.1 2007/01/22 16:36:33 huseyiner Exp $
 */
...
public class CurBLCurrentCardUpdate
{
	...
	public static void deleteCurrentCard(HashMap argMap) throws Exception
	{
		...
		Boolean hasTransaction = CurDALCurrentCardSearch.hasTransactions(currentCard);
		if(hasTransaction.booleanValue())
		{
			throw new TurquazException(CurLangKeys.MSG_CUR_CARD_HAS_TRANSACTIONS);
	...
	public static HashBag getCurrentcardInfo(HashMap argMap)throws Exception
	{
		...
		currentBag.put(CurKeys.CUR_DAYS_TO_VALUE,currentCard.getDaysToValue());
		...
		currentBag.put(CurKeys.CUR_CARD_BALANCES,new HashMap());
		List list = CurDALCurrentTransactionAdd.getTransactionTypes();
		it = list.iterator();
		i=0;
		while(it.hasNext())
		{
			TurqCurrentTransactionType type =(TurqCurrentTransactionType)it.next();
			
			List balanceList = CurDALCurrentCardUpdate.getCurrentTransactionBalances(type,curCardId);
			Object[] result =(Object[])balanceList.get(0);
			
			currentBag.put(CurKeys.CUR_CARD_BALANCES,i,EngKeys.TYPE_NAME,type.getTransactionTypeName());
			currentBag.put(CurKeys.CUR_CARD_BALANCES,i,EngKeys.DEPT_AMOUNT,result[0]);
			currentBag.put(CurKeys.CUR_CARD_BALANCES,i,EngKeys.CREDIT_AMOUNT,result[1]);
	...
	public static List getCurrentTransactionBalances(HashMap argMap) throws Exception
	{
		...
		return CurDALCurrentCardUpdate.getCurrentTransactionBalances(transType, curCardId);
	...
	public static void updateCurrentCard(HashMap argMap) throws Exception
	{
		...
		if(curCardId!=null)
		{
		 ...
		 String currentCode = (String)argMap.get(CurKeys.CUR_CURRENT_CODE);	
			...
			if(!currentCard.getCardsCurrentCode().equals(currentCode)&&CurDALCurrentCardAdd.isCurrentCodePresent(currentCode))
			{
