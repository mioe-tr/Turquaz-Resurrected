...
/**
 * @author Ehad Karacam
 * @version $Id: AccBLTransactionAdd.java,v 1.1 2007/01/22 16:36:35 huseyiner Exp $
 */
...
public class AccBLTransactionAdd
{
	...
	public static void saveAccTransactionFromUI(HashMap argMap)throws Exception
	{
		...
		TurqCurrencyExchangeRate exchangeRate = EngDALCommon.getCurrencyExchangeRate(currencyId,date);
		...
		saveAccTransaction(date, documentNo,type.intValue(),moduleId.intValue(),seqId,definition,exchangeRate,transColumns);
		
	}
	
	public static TurqAccountingTransaction saveInitialTransaction()throws Exception
	{
		...
		trans.setTransactionDocumentNo("");
		trans.setTransactionDescription(EngLangCommonKeys.STR_INITIAL_TRANSACTION);
		trans.setTransactionsDate(TurquazDateFormat.getFirstDayOfYear());
		/**
		 * TODO Will Change in next version
		 */
		trans.setTurqCurrencyExchangeRate(EngDALCommon.getBaseCurrencyExchangeRate());
		...
		transType.setId(new Integer(EngBLCommon.ACCOUNTING_TRANS_OPENING));
