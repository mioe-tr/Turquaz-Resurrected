 ...
 * @version $Id: AccDALTransactionSearch.java,v 1.1 2007/01/22 16:36:35 huseyiner Exp $
 */
...
public class AccDALTransactionSearch
{
	...
	public static List getSubsidiaryLedger(Integer accountIdStart, Integer accountIdEnd, Date startDate, Date endDate)throws Exception
	{
		...
		String query = "Select accounts.account_name as accName, accounts.account_code as accCode," +
				" topacc.account_name as topAccName, topAcc.account_code as topAccCode, transTable.id," +
				" transTable.rows_dept_in_base_currency, transTable.rows_credit_in_base_currency," +
				" transTable.transaction_definition, transTable.transactions_date, transTable.transaction_document_no" +
				" from turq_accounting_accounts topacc, turq_accounting_accounts accounts left join" +
				" (Select transCol.accounting_accounts_id ,transCol.id, transCol.rows_dept_in_base_currency," +
				" transCol.rows_credit_in_base_currency, transCol.transaction_definition, trans.transactions_date," +
				" trans.transaction_document_no from turq_accounting_transaction_columns transCol," +
				" turq_accounting_transactions trans where transCol.accounting_transactions_id=trans.id" +
				" and trans.transactions_date >=" + "'" + dformat.format(startDate) + "'" +
				" and trans.transactions_date <=" + "'" + dformat.format(endDate) + "' ) transTable ON" +
				" transTable.accounting_accounts_id=accounts.id" +
				" where accounts.top_account=topacc.id";
		if (accountEnd != null)
		{
		...
		query +=" order by accounts.account_code,transTable.transactions_date,transTable.transaction_document_no";		
	...
	public static List getGeneralLedger(Integer accountStartId, Integer accountEndId,Date startDate, Date endDate, boolean approved) throws Exception
	{
		try
		{
			...
			" accounts.account_code, transTable.transactions_date,transTable.transaction_document_no," + 
			" transTable.rows_dept_in_base_currency,transTable.rows_credit_in_base_currency,"+
			" transTable.transaction_definition, transTable.accounting_journal_id," +
			" accounts.id as accounting_accounts_id, transTable.columnId" + 
			" from turq_accounting_accounts accounts left join " +
			" (Select transcolumns.id as columnId,transcolumns.accounting_accounts_id, trans.transactions_date,trans.transaction_document_no," + 
			" transcolumns.rows_dept_in_base_currency,transcolumns.rows_credit_in_base_currency,"+
			" transcolumns.transaction_definition, trans.accounting_journal_id from turq_accounting_transactions trans," +
			" turq_accounting_transaction_columns transcolumns where transcolumns.accounting_transactions_id=trans.id" +
			" and trans.transactions_date >= '" + dformat.format(startDate) + "'" +
			" and trans.transactions_date <= '" + dformat.format(endDate) + "'" +
			" ) transTable ON transTable.accounting_accounts_id=accounts.id" +
			" where accounts.id <> -1 ";

			if (approved)
			{
				query += " and transTable.accounting_journal_id > 0";
			}
			
			TurqAccountingAccount accountStart = null;
			if (accountStartId != null)
			{
				accountStart = (TurqAccountingAccount) session.load(TurqAccountingAccount.class,
						accountStartId);
			}
			...
			query += " ORDER BY accounts.account_code,transTable.transactions_date";
			...
			throws Exception
	{
		try
		{
			...
			String query = "Select acc.accountCode," + "sum(transcolumn.rowsDeptInBaseCurrency),"
					...
					+ " and acc=transcolumn.turqAccountingAccount and acc.id <> -1";
			if (accountEnd == null && accountStart != null)
			{
				query += " and acc.id=" + accountStart.getId();
			}
			else if (accountEnd != null && accountStart != null)
			{
			...
			throws Exception
	{
		try
		{
			...
			String query = "select accounts.id , accounts.account_code, accounts.account_name, " +
					...
					+ " and accTrans.transactions_date >= '"+ df.format(startDate)+ "'"
			...
			{
				query += " where accounts.account_code >='" + firstAccount.getAccountCode() + "'"
						+ " and accounts.account_code <='" + secondAccount.getAccountCode() + "'";
			}
			else if (firstAccount != null)
			{
				query += " where accounts.account_code like '"+firstAccount.getAccountCode()+"%'";
			}
			Session session = EngDALSessionFactory.getSession();
			Statement statement = session.connection().createStatement();
			ResultSet rs = statement.executeQuery(query);
