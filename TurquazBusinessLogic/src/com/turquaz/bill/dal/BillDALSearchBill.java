...
{
	public static List searchBill(Integer curCard, String docNo, Date startDate, Date endDate, int type, Integer currencyId) throws Exception
	{
		try
		{
			...
			if (currencyId != null)
			{
				query += " and bill.turqCurrencyExchangeRate.turqCurrencyByExchangeCurrencyId.id="+currencyId;
			}
	...
	public static List getBillReport(TurqCurrentCard curCardStart, TurqCurrentCard curCardEnd, Date startDate, Date endDate,
			Date dueDateStart, Date dueDateEnd, BigDecimal minValue, BigDecimal maxValue, String docNoStart, String docNoEnd, int type, Integer currencyId)
			throws Exception
	{
		try
		{
			...
			String query = "Select bill.id, bill.billsDate, bill.billDocumentNo," +
					...
					" bill.turqCurrencyExchangeRate.turqCurrencyByExchangeCurrencyId.currenciesAbbreviation," +
					" billview.discountamount" +
					" from TurqBill as bill, TurqViewBillTransTotal as billview" +
					" where" + " bill.billsDate >= :startDate and bill.billsDate <= :endDate" +
					" and bill.id <> -1 " + " and bill.dueDate >= :dueDateStart and bill.dueDate <= :dueDateEnd" + 
