...
public class BillBLAddBill
{
    ...
    public static HashBag saveBillFromBill(HashMap argMap) throws Exception
    {
		...
		TurqCurrencyExchangeRate exchangeRate = EngDALCommon.getCurrencyExchangeRate(currencyId,billsDate);
        ...
        {
            ConBLAddConsignment.saveConsignmentFromBill(consDocNo, billDocNo, definition, isPrinted, consDate, type, currentCard,
                    exchangeRate, engSeq);
        }
		...
		resultBag.put(BillKeys.BILL_ID,bill.getId());
        return resultBag;
    }
    ...
    public static HashBag saveBillFromCons(HashMap argMap) throws TurquazException, Exception
    {
        try
        {
     ...
     * @throws Exception
     */
    private static TurqBill registerBill(TurqBill bill, String docNo, String definition, boolean isPrinted, Date billsDate, int type,
            Date dueDate, TurqCurrentCard curCard, TurqCurrencyExchangeRate exchangeRate, Integer billCheck, boolean isOpen)
            throws Exception
    {
        try
        {
    ...
    public static void saveCashTransaction(TurqBill bill, TurqCashCard cashCard, BigDecimal totalAmount) throws Exception
    {
        ...
        CashBLCashTransactionAdd.saveCashTransaction(cashCard, bill.getTurqEngineSequence(), type, bill.getBillsDate(), "FT. NO:"
                + bill.getBillDocumentNo(), bill.getBillDocumentNo(), totals, account, bill.getTurqCurrencyExchangeRate());
    ...
    public static void saveCurrentTransaction(TurqBill bill, BigDecimal totalAmount, BigDecimal discountAmount) throws Exception
    {
        try
        {
            ...
            String curTransDef = TurquazDateFormat.format(bill.getBillsDate()) + " " + bill.getBillDocumentNo() + " Ref. Fatura";
            // Al?? Faturas?
            if (bill.getBillsType() == EngBLCommon.COMMON_BUY_INT || bill.getBillsType() == EngBLCommon.COMMON_RETURN_SELL_INT)
            {
    ...
    private static int prepareAccountsForAccountingIntgretion(TurqBill bill, Map creditAccounts, Map deptAccounts) throws Exception
    {
        try
        {
            ...
            else if(bill.getBillsType()==EngBLCommon.COMMON_RETURN_BUY_INT)
            {
                invRows = creditAccounts;
                currentRows = deptAccounts;
               INV_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_BUY;
               INV_VAT_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_VAT_BUY;
               INV_SPEC_VAT_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_SPEC_VAT_BUY;
            ...
            else if(bill.getBillsType()==EngBLCommon.COMMON_RETURN_SELL_INT)
            {
                invRows = deptAccounts;
                currentRows = creditAccounts;
                INV_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_SELL;
                INV_VAT_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_VAT_SELL;
                INV_SPEC_VAT_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_SPEC_VAT_SELL;
                INV_DISCOUNT_ACCOUNT = EngBLCommon.INVENTORY_ACCOUNT_TYPE_RETURN_DISCOUNT_SELL;
            }
