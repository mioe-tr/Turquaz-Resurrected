...
/**
 * @author  Onsel
 * @version  $Id: CheUIOwnChequeCollect.java,v 1.1 2007/01/22 17:02:45 huseyiner Exp $
 */
...
public class CheUIOwnChequeCollect extends org.eclipse.swt.widgets.Composite implements SecureComposite
{
	{
	...
	public CheUIOwnChequeCollect(org.eclipse.swt.widgets.Composite parent, int style)
	{
		super(parent, style);
		initGUI();
	}
	...
	public void newForm()
	{
		CheUIOwnChequeCollect curCard = new CheUIOwnChequeCollect(this.getParent(), this.getStyle());
	...
	public void save()
	{
		try
		{
			if (verifyFields())
			{
				...
				EngTXCommon.doTransactionTX("Che_saveCollectOfOwnCheques",argMap);
								
				EngUICommon.showSavedSuccesfullyMessage(getShell()); //$NON-NLS-1$
				newForm();
			}
		}
		catch (Exception ex)
