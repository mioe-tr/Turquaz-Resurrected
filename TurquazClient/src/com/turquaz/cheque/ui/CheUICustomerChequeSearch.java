...
public class CheUICustomerChequeSearch extends org.eclipse.swt.widgets.Composite implements SearchComposite
{
	...
	private DatePicker dateReportDate;
	...
	private void initGUI()
	{
		try
		{
			...
			compSearchPanleLData.heightHint = 187;
			...
			currentPickerLData.widthHint = 404;
			...
			//END << comboStatus
			{
				lblReportDate = new Label(compSearchPanle, SWT.NONE);
				lblReportDate.setText("Rapor Tarihi");
			}
			{
				GridData dateReportDateLData = new GridData();
				dateReportDateLData.horizontalSpan = 3;
				dateReportDateLData.heightHint = 22;
				dateReportDateLData.widthHint = 150;
				dateReportDate = new DatePicker(compSearchPanle, SWT.NONE);
				dateReportDate.setLayoutData(dateReportDateLData);
			}
			//START >> lblDueDate
			...
			tableColumnChequeNo.setWidth(116);
	...
	public void search()
	{
		...
		try
		{
			...
			argMap.put(CheKeys.CHE_REPORT_DATE,dateReportDate.getDate());
			
			if (map.containsKey(comboStatus.getText()))
			{
				argMap.put(CheKeys.CHE_STATUS_STRING,comboStatus.getText());
			}
			else
			{
				argMap.put(CheKeys.CHE_STATUS_STRING,EngLangCommonKeys.COMMON_ALL_STRING);
			}
			...
			HashBag chequeBag = (HashBag)EngTXCommon.doSelectTX("Che_getCustomerChequeAbstract",argMap);
		 
			JasperPrint jasperPrint=(JasperPrint)chequeBag.get(EngKeys.REPORT);
			HashMap chequeList =(HashMap)chequeBag.get(CheKeys.CHE_CHEQUES);			
