...
public class InvUITransactionTableRow implements ITableRow
{
	...
	public void fillAfterSetDB()
	{
		try
		{
			...
			HashBag cardBag = (HashBag) EngTXCommon.doSelectTX("Inv_initializeInventoryCard", argMap);
	...
	public void fillDefaults(HashMap invCard)
	{
		...
		{
			HashMap argMap = new HashMap();
			Integer invCardId=(Integer)invCard.get(InvKeys.INV_CARD_ID);
			...
			Integer currencyId=(Integer)tableViewer.getTable().getData(EngKeys.CURRENCY_ID);
			BigDecimal unitPrice=EngBLInventoryCards.getCardValue(invCardId,currencyId,transType);
			
			invTrans.put(InvKeys.INV_UNIT_PRICE_IN_FOREIGN_CURRENCY, unitPrice);
