 ...
 * @version  $Id: InvUICardSearchDialog.java,v 1.1 2007/01/22 17:02:33 huseyiner Exp $
 */
...
public class InvUICardSearchDialog extends org.eclipse.swt.widgets.Dialog
{
	...
	public void comboWidgetSelected()
	{
		...
		if (invMainGr != null)
		{
			...
			for(int k=0; k<subGroups.size(); k++)
			{
				...
				comboSubGroup.add(invGrName);
				comboSubGroup.setData(invGrName, invGr);
			}
	...
	{		
		try
		{
			...
			HashMap grMap=(HashMap)comboSubGroup.getData(comboSubGroup.getText());
			...
			HashBag cardBag =(HashBag)EngTXCommon.doSelectTX("Inv_searchCards",argMap);
			...
			for (int k = 0; k < cards.size(); k++)
			{
				...
				TableItem item = new TableItem(tableSearcResults, SWT.NULL);
				item.setData(invCode);
				
				item.setText(new String[]{invCode, invName,""});
			}
		}
		catch (Exception ex)
		{
