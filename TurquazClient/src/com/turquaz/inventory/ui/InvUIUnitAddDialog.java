 ...
 * @version  $Id: InvUIUnitAddDialog.java,v 1.1 2007/01/22 17:02:34 huseyiner Exp $
 */
...
public class InvUIUnitAddDialog extends org.eclipse.swt.widgets.Dialog
{
	...
	public void fillTable()
	{
		try
		{
			...
			HashBag unitBag = (HashBag) EngTXCommon.doSelectTX("Inv_getInventoryUnits",null);
			...
			{
				HashMap unitInfo = (HashMap) unitList.get(new Integer(i));
				item = new TableItem(tableInvUnits, SWT.NULL);
				item.setText(new String[]{unitInfo.get(InvKeys.INV_UNIT_NAME).toString()});
				item.setData(unitInfo.get(InvKeys.INV_UNIT_ID));
			}
		}
		catch (Exception ex)
		{
	...
	protected void btnDeleteMouseUp(MouseEvent evt)
	{
		try
		{
			...
			if (okToDelete)
			{
				...
				argMap.put(InvKeys.INV_UNIT_ID,txtUnitName.getData());
				//XXX check if there exist a transaction with this unit!!
				EngTXCommon.doTransactionTX("Inv_deleteInvUnit",argMap);
				btnDelete.setEnabled(false);
				btnUpdate.setEnabled(false);
	...
	protected void btnUpdateMouseUp(MouseEvent evt)
	{
		try
		{
			...
			else
			{
				...
				{
					HashMap unitInfo = (HashMap) unitList.get(new Integer(k));
					
					if (unitInfo.get(InvKeys.INV_UNIT_NAME).equals(unit))
					{
						exist = true;
						break;
					}
				...
				EngTXCommon.doTransactionTX("Inv_updateInvUnit",argMap);
				btnDelete.setEnabled(false);
				btnUpdate.setEnabled(false);
				btnUnitAdd.setEnabled(true);
				txtUnitName.setText(""); //$NON-NLS-1$
	...
	protected void btnUnitAddMouseUp()
	{
		try
		{
			...
			HashMap argMap=new HashMap();
			argMap.put(InvKeys.INV_UNIT_NAME,txtUnitName.getText().trim());
			EngTXCommon.doTransactionTX("Inv_saveUnit",argMap);
			txtUnitName.setText(""); //$NON-NLS-1$
