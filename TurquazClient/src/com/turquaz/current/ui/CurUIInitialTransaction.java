...
public class CurUIInitialTransaction extends org.eclipse.swt.widgets.Composite
{
	...
	final static String CURRENT_CARD = CurLangKeys.STR_CUR_CARD;
	final static String DEPT = EngLangCommonKeys.STR_DEPT;
	final static String CREDIT = EngLangCommonKeys.STR_CREDIT;
	TableCursor cursor;
	...
	public void fillTable()
	{
		try
		{
			HashBag transBag =(HashBag)EngTXCommon.doSelectTX("Cur_getInitialTransactions",null);
		...
		rowList.addChangeListener(new ITableRowListViewer()
		{
			public void updateRow(ITableRow row)
			{
				try
				{
					...
					EngTXCommon.doTransactionTX("Cur_updateInitialTransaction",argMap);
				}
				catch (Exception ex)
				{
                    EngBLLogger.log(this.getClass(),ex,getShell());
