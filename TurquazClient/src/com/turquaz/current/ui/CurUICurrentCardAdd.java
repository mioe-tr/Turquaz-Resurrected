 ...
 * @version  $Id: CurUICurrentCardAdd.java,v 1.1 2007/01/22 17:02:40 huseyiner Exp $
 */
...
public class CurUICurrentCardAdd extends Composite implements SecureComposite
{
	...
	public void fillGroups()
	{
		try
		{
			...
			HashBag groupBag = (HashBag)EngTXCommon.doSelectTX("Cur_getCurrentGroups",null); //$NON-NLS-1$
	...
	public boolean verifyFields(boolean save) throws Exception
	{
		try
		{
			...
			if (accPickerCustomer.getId() == null)
			{
				...
				if (ans)
				{
					String accountCode = new AccUIAddAccountDialog(getShell(), SWT.NONE).open();
					accPickerCustomer.setText(accountCode);
				}
			}
			/*
	...
	public void save()
	{
		try
		{
			if (verifyFields(true))
			{
				...
				EngTXCommon.doTransactionTX("Cur_saveCurrentCard",argMap);	 //$NON-NLS-1$
