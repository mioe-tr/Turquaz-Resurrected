 ...
 * @version  $Id: CurUITransactionSearch.java,v 1.1 2007/01/22 17:02:40 huseyiner Exp $
 */
...
public class CurUITransactionSearch extends Composite implements SearchComposite
{
	...
	public void initGUI()
	{
		try
		{
			...
			this.setLayout(thisLayout);
			{
                ...
                //START >>  lblTransType
                lblTransType = new CLabel(composite1, SWT.NONE);
                lblTransType.setText("Hareket Tipi");
                //END <<  lblTransType
                //START >>  comboTransType
                GridData comboTransTypeLData = new GridData();
                comboTransTypeLData.widthHint = 120;
                comboTransTypeLData.heightHint = 17;
                comboTransType = new CCombo(composite1, SWT.NONE);
				...
				}
				{
					...
					txtDefinitionLData.widthHint = 118;
	...
	public void postInitGUI()
	{
        ...
        // fill type combo 
        comboTransType.removeAll();
        // all
        comboTransType.add(EngLangCommonKeys.COMMON_ALL_STRING);
        comboTransType.setData(EngLangCommonKeys.COMMON_ALL_STRING,null);
        // buy 0 
        comboTransType.add(EngLangCommonKeys.COMMON_DEPT_STRING);
        comboTransType.setData(EngLangCommonKeys.COMMON_DEPT_STRING,new Integer(0));
        // sell 1
        comboTransType.add(EngLangCommonKeys.COMMON_CREDIT_STRING);
        comboTransType.setData(EngLangCommonKeys.COMMON_CREDIT_STRING, new Integer (1));
        
        comboTransType.setText(EngLangCommonKeys.COMMON_ALL_STRING);
	...
	public void fillComboTypes()
	{
		try
		{
			...
			HashBag resultBag =(HashBag)EngTXCommon.doSelectTX("Cur_getCurrentTransactionTypes",null);
					
			HashMap types = (HashMap)resultBag.get(EngKeys.TYPES);
			
			
			comboTransactionGroup.add(EngLangCommonKeys.COMMON_ALL_STRING);
			comboTransactionGroup.setData(EngLangCommonKeys.COMMON_ALL_STRING, null);
	...
	public void search()
	{
		try
		{
			...
			argMap.put(CurKeys.CUR_TRANS_TYPE,comboTransType.getData(comboTransType.getText()));
			
			HashBag resultBag = (HashBag)EngTXCommon.doSelectTX("Cur_searchCurrentTransaction",argMap); 
	...
	public void createTableViewer()
	{
		...
		columnTypes[8] = TurquazTableSorter.COLUMN_TYPE_STRING;
		...
		try
		{
			...
			if (items.length > 0)
			{
				...
				if (transId != null)
				{
					...
					{
						HashBag transInfo = (HashBag)EngTXCommon.doSelectTX("Cur_getBankTransaction",argMap);
					...
					{
						HashBag result = (HashBag)EngTXCommon.doSelectTX( "Cur_getBillofCurrentTrans",argMap);
					...
					{
						HashBag result = (HashBag)EngTXCommon.doSelectTX("Cur_getCheqeuTransaction",argMap);
					...
					{
						HashBag result = (HashBag)EngTXCommon.doSelectTX("Cur_getCashTransaction",argMap);
