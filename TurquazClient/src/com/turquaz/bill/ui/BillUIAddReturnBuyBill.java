 ...
 * @version  $Id: BillUIAddReturnBuyBill.java,v 1.1 2007/01/22 17:02:39 huseyiner Exp $
 */
...
public class BillUIAddReturnBuyBill extends Composite implements SecureComposite
{
    ...
    private final int BILL_TYPE = EngBLCommon.COMMON_RETURN_BUY_INT;
                    ...
                    tabItemGeneral.setImage(SWTResourceManager.getImage("icons/Home16.gif")); //$NON-NLS-1$
                    {
                        ...
                        tabItemGeneral.setControl(compGeneral);
                        {
                            ...
                            }
                            {
                                ...
                                dateConsignmentDateLData.widthHint = 141;
                                dateConsignmentDateLData.heightHint = 24;
                            ...
                            //END <<  lblConsDate
                            {
                                ...
                                dateDueDateLData.widthHint = 140;
                                dateDueDateLData.heightHint = 24;
                            ...
                            }
                            {
                                ...
                                comboWareHouseLData.widthHint = 119;
							...
							comboCurrencyType.addSelectionListener(new SelectionAdapter() {
								public void widgetSelected(SelectionEvent evt) {
									tableViewer.getTable().setData(EngKeys.CURRENCY_ID,comboCurrencyType.getData(comboCurrencyType.getText().trim()));
								}
							});
    ...
    public void fillComboWarehouses()
    {
        try
        {
			...
			HashBag wareHouseBag = (HashBag) EngTXCommon.doSelectTX("Inv_getInventoryWarehouses", null);
    ...
    public void fillGroupsTable()
    {
        ...
        {
//			 Fill Group Table
			HashBag groupBag = (HashBag) EngTXCommon.doSelectTX("Bil_getBillGroups", null);
    ...
    public void createTableViewer()
    {
        ...
        editors[0] = new InventoryCellEditor(tableConsignmentRows,INVENTORY_CODE); //Stok Kodu
        editors[1] = new InventoryNameCellEditor(tableConsignmentRows,INVENTORY_NAME); //Stok Adi
		...
		((InventoryCellEditor)editors[0]).setTableViewer(tableViewer.getViewer());
		((InventoryNameCellEditor)editors[1]).setTableViewer(tableViewer.getViewer());
    ...
    public boolean verifyFields()
    {
        try
        {
            ...
            if (!checkStabilityInventoryLevel())
			{
				return false;
			}
	...
	public boolean checkStabilityInventoryLevel()
    {
        try
        {
			...
			for (int i = 0; i < items.length; i++)
			{
				...
				if (row.okToSave())
				{
					...
					invInfo[0] = invCard.get(InvKeys.INV_CARD_ID);
					invInfo[1] = invTrans.get(InvKeys.INV_AMOUNT_OUT);
					invList.add(invInfo);
				}
			 }
						
			HashMap argMap=new HashMap();
			argMap.put(InvKeys.INV_CARDS, invList);
			argMap.put(EngKeys.TYPE, new Integer(EngBLCommon.COMMON_RETURN_BUY_INT));
			HashBag checkBag=(HashBag)EngTXCommon.doSelectTX("Inv_checkStabilityInventoryLevel",argMap);
		
			List warningList=(List)checkBag.get(InvKeys.INV_CARDS);
			
			if (warningList.size() > 0)
			{
				String invMes="";
				for(int k=0;k<warningList.size(); k++)
				{
					String invCode=(String)warningList.get(k);
					invMes += "-"+invCode+"\n";
				}
				boolean sellCont=EngUICommon.showQuestion(getShell(),InvLangKeys.MSG_WARNING_UNDER_THE_MIN_LEVEL_INV_CARD+invMes+InvLangKeys.MSG_ARE_YOU_WILLING_TO_CONTINUE);
				return sellCont;
			}
			return true;
    ...
    public void save()
    {
        try
        {
            if (verifyFields())
            {
				...
				HashBag resultbag = (HashBag) EngTXCommon.doTransactionTX("Bil_saveBillFromBill", argMap);
				...
				Integer billId=(Integer)resultbag.get(BillKeys.BILL_ID);
		
				
				boolean printBill=EngUICommon.showQuestion(getShell(),BillLangKeys.MSG_WANT_TO_PRINT_BILL);
				if (printBill)
				{
					boolean printBalance=EngUICommon.showQuestion(getShell(),BillLangKeys.MSG_WILL_BALANCE_BE_PRINTED);
					EngBLUtils.printBill(billId,printBalance);
				}
