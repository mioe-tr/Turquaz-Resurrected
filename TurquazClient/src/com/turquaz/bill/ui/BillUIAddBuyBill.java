 ...
 * @version  $Id: BillUIAddBuyBill.java,v 1.1 2007/01/22 17:02:39 huseyiner Exp $
 */
...
public class BillUIAddBuyBill extends Composite implements SecureComposite {
	...
	private void initGUI() {
		try {
					...
					tabItemGeneral.setImage(SWTResourceManager.getImage("icons/Home16.gif")); //$NON-NLS-1$
					{
						...
						tabItemGeneral.setControl(compGeneral);
						{
							...
							}
							{
								...
								txtCurrentCardLData.widthHint = 400;
							...
							}
							{
								...
								dateConsignmentDateLData.widthHint = 150;
								dateConsignmentDateLData.heightHint = 22;
							...
							// END << lblConsDate
							{
								...
								dateDueDateLData.widthHint = 150;
							...
							}
							{
								...
								comboWareHouseLData.widthHint = 130;
			...
			tableConsignmentRows.setEnabled(true);
			this.layout();
			postInitGui();
	...
	public void createTableViewer() {
		...
		editors[1] = new InventoryNameCellEditor(tableConsignmentRows,INVENTORY_NAME); // Stok
																		// Adi
        ...
        try
        {
			...
			for (int i = 0; i < items.length; i++)
			{
				...
				if (row.okToSave())
				{
					...
					invInfo[1] = invTrans.get(InvKeys.INV_AMOUNT_IN);
					invList.add(invInfo);
				}
			 }
			...
			argMap.put(EngKeys.TYPE, new Integer(EngBLCommon.COMMON_BUY_INT));
			...
			if (warningList.size() > 0)
			{
				...
				boolean buyCont=EngUICommon.showQuestion(getShell(),InvLangKeys.MSG_WARNING_OVER_THE_MAX_LEVEL_INV_CARD+invMes+InvLangKeys.MSG_ARE_YOU_WILLING_TO_CONTINUE);
				return buyCont;
			}
			return true;
