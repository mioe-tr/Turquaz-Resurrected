 ...
 * @version  $Id: BillUIBillFromConsignment.java,v 1.1 2007/01/22 17:02:39 huseyiner Exp $
 */
					...
					tabItemGeneral.setImage(SWTResourceManager.getImage("icons/Home16.gif")); //$NON-NLS-1$
					{
						...
						//END <<  toolBar
						{
							...
							}
							{
								...
								dateConsDateLData.widthHint = 130;
							...
							}
							{
								...
								dateConsignmentDateLData.widthHint = 130;
							...
							}
							{
								...
								dateDueDateLData.widthHint = 130;
							...
							}
							{
								...
								comboCurrencyType.setEnabled(false);
							}
							{
							...
							}
							{
								...
								comboConsignmentTypeLData.widthHint = 110;
...
//			 Fill Group Table
	...
	public void calculateTotals()
	{
		...
		{
			HashMap transInfo = (HashMap) (items[i].getData());
			if (transInfo != null)
			{
				subTotal = subTotal.add((BigDecimal) transInfo
						.get(InvKeys.INV_TOTAL_PRICE_IN_FOREIGN_CURRENCY));
				totalVAT = totalVAT
						.add((BigDecimal) transInfo.get(InvKeys.INV_VAT_AMOUNT_IN_FOREIGN_CURRENCY));
				totalSpecVAT = totalSpecVAT.add((BigDecimal) transInfo
						.get(InvKeys.INV_VAT_SPECIAL_AMOUNT_IN_FOREIGN_CURRENCY));
				discountTotal = discountTotal.add((BigDecimal) transInfo
	...
	public void save()
	{
		try
		{
			if (verifyFields())
			{
				...
				HashBag resultBag=(HashBag)EngTXCommon.doTransactionTX("Bil_saveBillFromCons",argMap);
	...
	public void chooseConsignmentMouseUp()
	{
		...
		try
		{
			Object[] result = new ConUIConsignmentSearchDialog(this.getShell(), SWT.NULL).open(dateBillDate
					.getDate());
			...
			String curName=(String)result[2];
			Integer currencyId=(Integer)result[3];
			String currencyAbbr=(String)result[4];
			txtCurrentCard.setData(curCardId);
			consList = (List) result[1];
			tableConsignmentRows.removeAll();
			
			comboConsignmentType.add(currencyAbbr);
			comboCurrencyType.setText(currencyAbbr);
			comboCurrencyType.setData(currencyAbbr,currencyId);
			
			txtCurrentCard.setText(curName);
			txtCurrentCard.setData(curCardId);
			...
			for (int i = 0; i < consList.size(); i++)
			{
				...
				for (int k = 0; k < invTransMap.size(); k++)
				{
					...
					item.setText(new String[]{invCardInfo.get(InvKeys.INV_CARD_NAME).toString(),
							...
							invTransInfo.get(InvKeys.INV_UNIT_NAME).toString(),
							invTransInfo.get(InvKeys.INV_UNIT_PRICE_IN_FOREIGN_CURRENCY).toString(),
							invTransInfo.get(InvKeys.INV_TOTAL_PRICE_IN_FOREIGN_CURRENCY).toString(),
							invTransInfo.get(InvKeys.INV_VAT_RATE) + "", //$NON-NLS-1$
				...
				String docNo=consInfo.get(EngKeys.DOCUMENT_NO).toString();
				if (!docNo.equals(""))
				{
					if (!txtDocumentNo.getText().equals(""))
					{
						txtDocumentNo.setText(txtDocumentNo+","+docNo);
					}
					else
					{
						txtDocumentNo.setText(docNo);
					}					
				}				
				calculateTotals();
			}
