...
/**
 * @author Onsel
 * @version $Id: TurquazContentAssistProcessors.java,v 1.1 2007/01/22 17:02:51 huseyiner Exp $
 */
...
public class TurquazContentAssistProcessors implements ISubjectControlContentAssistProcessor
{
	...
	public static Proposal[] CONTENT_ASSIST_NORMAL_ACCOUNTS;
	...
	public static Proposal[][] proposedCodeList = new Proposal[][]{ACCOUNTS,
			...
			CONTENT_ASSIST_NORMAL_ACCOUNTS,
	...
	public static void fillProposalArray(HashMap argMap) throws Exception
	{
		try
		{
			...
			else if (type == EngBLCommon.CONTENT_ASSIST_NORMAL_ACCOUNTS)
			{
				fillAccountingModuleArray();
			}
	...
	public static void fillAccountingModuleArray() throws Exception
	{
		...
		proposedCodeList[EngBLCommon.CONTENT_ASSIST_NORMAL_ACCOUNTS] = new Proposal[proposed.size()];
		proposed.toArray(proposedCodeList[EngBLCommon.CONTENT_ASSIST_NORMAL_ACCOUNTS]);
	...
	{
		List proposedCode = new ArrayList();
		List proposedName=new ArrayList();
		HashMap list = EngBLInventoryCards.getInventoryCards();
		Iterator it=list.keySet().iterator();
		while(it.hasNext())
		{
			String invCode=(String)it.next();
			HashMap cardInfo = (HashMap) list.get(invCode);
			String invName=(String)cardInfo.get(InvKeys.INV_CARD_NAME);
			proposedCode.add(new Proposal(invCode,invName));
			proposedName.add(new Proposal(invName,invCode));
		}
		proposedCodeList[EngBLCommon.CONTENT_ASSIST_INVENTORY] = new Proposal[proposedCode.size()];
		proposedCode.toArray(proposedCodeList[EngBLCommon.CONTENT_ASSIST_INVENTORY]);
		
		proposedCodeList[EngBLCommon.CONTENT_ASSIST_INVENTORY_NAME] = new Proposal[proposedName.size()];
		proposedName.toArray(proposedCodeList[EngBLCommon.CONTENT_ASSIST_INVENTORY_NAME]);
	...
	public static void fillCashModuleArray() throws Exception
	{
		...
		while (it.hasNext())
		{
			HashMap cardInfo = (HashMap) (list.get(it.next()));
