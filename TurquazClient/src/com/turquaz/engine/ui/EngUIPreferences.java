...
public class EngUIPreferences extends org.eclipse.swt.widgets.Dialog
{
	...
	private Button radioZIP;
	private Button radioLZMA;
	private Group groupCompression;
	...
	public void open()
	{
		try
		{
				...
				toolBar1 = new ToolBar(dialogShell, SWT.NONE);
				{
					...
					toolSave.addSelectionListener(new SelectionAdapter()
					{
						public void widgetSelected(SelectionEvent evt)
						{
							...
							EngCompression.setCompressionMethod((radioLZMA.getSelection() ? Compression.LZMA : Compression.ZIP));
			...
			}
			{
				...
				{
					groupCompression = new Group(compGeneral, SWT.NONE);
					GridLayout groupCompressionLayout = new GridLayout();
					groupCompressionLayout.makeColumnsEqualWidth = true;
					groupCompression.setLayout(groupCompressionLayout);
					GridData groupCompressionLData = new GridData();
					groupCompressionLData.widthHint = 227;
					groupCompressionLData.heightHint = 56;
					groupCompressionLData.horizontalSpan = 2;
					groupCompression.setLayoutData(groupCompressionLData);
					groupCompression.setText("S\u0131k\u0131\u015ft\u0131rma Metodu");
					{
						radioLZMA = new Button(groupCompression, SWT.RADIO | SWT.LEFT);
						radioLZMA.setText("LZMA [Internet]");
						radioLZMA.setSelection(true);
					}
					{
						radioZIP = new Button(groupCompression, SWT.RADIO | SWT.LEFT);
						radioZIP.setText("ZIP [LAN]");
					}
				}
				//END <<  compGeneral
	...
	{
		int compression=EngCompression.getCompressionMethod();
		if (compression==Compression.LZMA)
		{
			radioLZMA.setSelection(true);
			radioZIP.setSelection(false);
		}
			
		else
		{
			radioZIP.setSelection(true);
			radioLZMA.setSelection(false);
		}
	}
	...
	private void fillBillTypeCombo()
	{
		try
		{
			if (EngConfiguration.getString("invoice_template") != null)
			{
				String invoice_template = EngConfiguration.getString("invoice_template");
				if (invoice_template.endsWith(".jasper"))
				{
					invoice_template = invoice_template.substring(0, invoice_template.length() - 7)
							+ ".jrxml";
				}
				cCombo.setText(invoice_template);
			}
			HashBag fileBag = (HashBag) EngTXCommon.doTransactionTX("Util_getInvoiceTemplates", null);
			HashMap fileMap = (HashMap) fileBag.get(EngKeys.BILL_TEMPLATES);
			for (int i = 0; i < fileMap.size(); i++)
			{
				String filename = (String) fileMap.get(new Integer(i));
				cCombo.add(filename);
			}
		}
		catch (Exception ex)
