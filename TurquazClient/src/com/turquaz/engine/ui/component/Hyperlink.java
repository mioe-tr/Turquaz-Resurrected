...
/**
 * @author  Onsel
 * @version  $Id: Hyperlink.java,v 1.1 2007/01/22 17:02:37 huseyiner Exp $
 */
 ...
 * <p>
 * This control displays a line of text (with an optional underline) which can be clicked to send a Selection event. Colors for the text and
 * underline in their normal, mouse hover and active state can be set independently. The text can contain a mnemonic character for
 * triggering the link via keyboard. Unless the control is created with the NO_FOCUS style, it accepts keyboard focus and can be triggered
 * with RETURN and SPACE while focused.
 * </p>
 * <p>
...
public final class Hyperlink extends Canvas
{
	...
	public Hyperlink(Composite parent, int style)
	{
		...
		Listener mouseListener = new Listener()
		{
			public void handleEvent(Event event)
			{
				...
				if (cursorInControl != newCursorInControl)
				{
					...
					if (isActive || (normalForeground != hoverForeground) || (normalUnderline != hoverUnderline))
						redraw();
				}
			}
		};
		...
		// Don't take focus but still support mnemonics
		{
			addListener(SWT.Traverse, new Listener()
			{
				public void handleEvent(Event event)
		...
		while (shellComp != null && (!(shellComp instanceof Shell)))
			shellComp = shellComp.getParent();
	...
	{
		return event.x >= cachedClientArea.x && event.x < cachedClientArea.x + cachedClientArea.width && event.y >= cachedClientArea.y
				&& event.y < cachedClientArea.y + cachedClientArea.height;
	}
	...
	private static int checkStyle(int style)
	{
		...
		//TEMPORARY CODE
		/*
		 * The default background on carbon and some GTK themes is not a solid color but a texture. To show the correct default
		 * background, we must allow the operating system to draw it and therefore, we can not use the NO_BACKGROUND style. The
		 * NO_BACKGROUND style is not required on platforms that use double buffering which is true in both of these cases.
		 */
	...
	private void onPaint(PaintEvent event)
	{
		...
		if (textFG == null)
			textFG = normalForeground;
		if (textFG == null)
			textFG = getDisplay().getSystemColor(SWT.COLOR_WIDGET_FOREGROUND);
	 ...
	 * <p>
	 * Mnemonics are indicated by an '&amp' that causes the next character to be the mnemonic. When the user presses a key sequence that
	 * matches the mnemonic, a selection event occurs. On most platforms, the mnemonic appears underlined but may be emphasised in a
	 * platform specific manner. The mnemonic indicator character '&amp' can be escaped by doubling it in the string, causing a single
	 * '&amp' to be displayed.
	 * </p>
	 * 
	 * @param text
	 *             the text to be displayed in the hyperlink or null
	 * @exception SWTException
	 *                 <ul>
