...
public class SWTPTree
{
	...
	protected void fillDocument()
	{
		...
		for (int j = 0; j < tree.getColumnCount(); j++)
		{
			...
			int column_width = tree.getColumn(j).getWidth();
			if (column_width < 20)
			{
				continue;
			}
			...
			PBox box = boxProvider.createBox(parent, style, j, 0, column_width, height, true, tree.getColumn(j).getText());
		...
		for (int i = 0; i < tree.getItemCount(); i++)
		{
			...
			for (int j = 0; j < tree.getColumnCount(); j++)
			{
				...
				if (j == 0)
					style = PBox.POS_BELOW | PBox.ROW_ALIGN;
				PBox box = boxProvider.createBox(parent, style, j, i, column_width, height, false, tree.getItem(i).getText(j));
				if (tree.getColumn(j).getStyle() == SWT.RIGHT)
				{
	...
	public void calculatePageLengths()
	{
		if (tree != null)
		{
			...
			for (int j = 0; j < tree.getColumnCount(); j++)
			{
				double boxWidth = Math.max(0, tree.getColumn(j).getWidth() * 0.03);
				width -= boxWidth;
				if (width < 0)
				{
					break;
	...
	public void setBoxProvider(PTableBoxProvider boxProvider)
	{
		...
		if (this.boxProvider != null && this.tree != null)
		{
			fillDocument();
		}
	}
	 ...
	 *             The table to set
	 */
	public void setTree(Tree table)
	{
		this.tree = table;
		if (this.boxProvider != null && this.tree != null)
		{
