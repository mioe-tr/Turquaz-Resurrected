...
/**
 * Like a regular SashForm, the LiveSashForm lays out its children in a Row or Column arrangement (as specified by the orientation) and
 * places a Sash between the children. One child may be maximized to occupy the entire size of the LiveSashForm. The relative sizes of the
 * children may be specfied using weights.
 * <p>
 * LiveSashForm supports the following additional features:
 * <ul>
 * <li>Unless the LiveSashForm is created with the NO_LIVE_UPDATE style, the bounds of the children are updated in real time when a Sash is
 * being dragged.</li>
 * <li>Borders can be assigned to the individual child widgets by calling <code>setChildBorder()</code>. This has the same effect as
 * placing the child widgets in FramedComposites except the borders become part of the adjacent sashes, thus making the draggable part
 * bigger without consuming additional screen real estate.</li>
 * </ul>
 * </p>
 * <p>
 * <dl>
 ...
 * 
 * @since Feb 15, 2004
 * @version $Id: LiveSashForm.java,v 1.1 2007/01/22 17:02:38 huseyiner Exp $
 */
public class LiveSashForm extends Composite
{
	 ...
	 * <p>
	 * The style value is either one of the style constants defined in class <code>SWT</code> which is applicable to instances of this
	 * class, or must be built by <em>bitwise OR</em> 'ing together (that is, using the <code>int</code> "|" operator) two or more of
	 * those <code>SWT</code> style constants. The class description lists the style constants that are applicable to the class. Style
	...
	/**
	 * Returns SWT.HORIZONTAL if the controls in the SashForm are laid out side by side or SWT.VERTICAL if the controls in the SashForm are
	...
	/**
	 * Answer the relative weight of each child in the SashForm. The weight represents the percent of the total width (if SashForm has
	 * Horizontal orientation) or total height (if SashForm has Vertical orientation) each control occupies. The weights are returned in
	 * order of the creation of the widgets (weight[0] corresponds to the weight of the first child created).
	 * 
	...
	/**
	 * If orientation is SWT.HORIZONTAL, lay the controls in the SashForm out side by side. If orientation is SWT.VERTICAL, lay the
	...
	/**
	 * Specify the control that should take up the entire client area of the SashForm. If one control has been maximized, and this method
	 * is called with a different control, the previous control will be minimized and the new control will be maximized.. if the value of
	 * control is null, the SashForm will minimize all controls and return to the default layout where all controls are laid out separated
	 * by sashes.
	 * 
	 * @param control
	...
	/**
	 * Specify the relative weight of each child in the SashForm. This will determine what percent of the total width (if SashForm has
	 * Horizontal orientation) or total height (if SashForm has Vertical orientation) each control will occupy. The weights must be
	 * positive values and there must be an entry for each non-sash child of the SashForm.
	 * 
	 * @param weights
	 ...
	 *                 <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed</li>
	 *                 <li>ERROR_THREAD_INVALID_ACCESS - if not called from the thread that created the receiver</li>
	 *                 <li>ERROR_INVALID_ARGUMENT - if the weights value is null or of incorrect length (must match the number of
	 *                 children)</li>
	 *                 </ul>
	 */
	 ...
	 * @param shadow
	 *             One of the SWT shadow constants SHADOW_IN, SHADOW_OUT, SHADOW_ETCHED_IN, SHADOW_ETCHED_OUT, SHADOW_NONE.
	 */
	 ...
	 * @return One of the SWT shadow constants SHADOW_IN, SHADOW_OUT, SHADOW_ETCHED_IN, SHADOW_ETCHED_OUT, SHADOW_NONE.
	 */
