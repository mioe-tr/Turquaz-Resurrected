 ...
 * @version $Id: EngBLAccountingAccounts.java,v 1.1 2007/01/22 17:02:51 huseyiner Exp $
 */
...
public class EngBLAccountingAccounts
{
	...
	public void fillAccountList() throws Exception
	{
			...
			HashBag resultBag =(HashBag)EngTXCommon.doSelectTX("Acc_getAccountingAccountLists",null);
			...
			mainAccounts = (List)resultBag.get(AccKeys.ACC_MAIN_ACCOUNTS);
			
			
			accountMap.clear();
			leafAccountMap.clear();
			...
			for(int k=0; k<mainAccounts.size(); k++)
			{
				HashMap accountInfo = (HashMap)mainAccounts.get(k);
				mainAccountMap.put(accountInfo.get(AccKeys.ACC_ACCOUNT_CODE), accountInfo);
			}
	...
	public static HashMap getMainAccount(String accountCode) throws Exception
	{
		try
		{
			if (_instance == null)
			...
			return (HashMap) _instance.mainAccountMap.get(accountCode);
		}
		catch (Exception ex)
		{
			throw ex;
