 ...
 * @version  $Id: AccUIInitialTransaction.java,v 1.1 2007/01/22 17:02:49 huseyiner Exp $
 */
...
public class AccUIInitialTransaction extends Composite implements SecureComposite
{
	...
	public void postInitGUI()
	{
		try
		{
			accountBag = (HashBag) EngTXCommon.doSelectTX("Acc_getInitialTransaction", null);
			createTableViewer();
		...
		try
		{
			if (accountBag != null)
			{
				...
				for(int k=0; k<transRows.size(); k++)
				{
					...
					HashMap accountInfo = (HashMap)transRow.get(AccKeys.ACC_ACCOUNT);
					
					String accountCode = (String)accountInfo.get(AccKeys.ACC_ACCOUNT_CODE);
