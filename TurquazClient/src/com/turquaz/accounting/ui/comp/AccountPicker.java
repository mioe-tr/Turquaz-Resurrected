 ...
 */
public class AccountPicker extends org.eclipse.swt.widgets.Composite implements TurquazContentAssistInterface
{
	{
			...
			this.setLayout(thisLayout);
			{
				...
				modifyListener=new ModifyListener()
				{
					public void modifyText(ModifyEvent evt)
					{
						try
						{
							HashMap accMap=EngBLAccountingAccounts.getAccount(text1.getText().trim());
							setDataMap(accMap);							
						}
						catch (Exception ex)
						{
	...
	public void postInitGUI()
	{
		...
		final SubjectControlContentAssistant asistant = new TurquazContentAssistant(adapter,
				EngBLCommon.CONTENT_ASSIST_NORMAL_ACCOUNTS);
	}
