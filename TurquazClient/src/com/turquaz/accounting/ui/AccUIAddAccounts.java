 ...
 * @version  $Id: AccUIAddAccounts.java,v 1.1 2007/01/22 17:02:49 huseyiner Exp $
 */
...
public class AccUIAddAccounts extends Composite implements SecureComposite
{
	...
	public boolean verifyFields()
	{
		try
		{
			if (txtParentAccount.getData() == null)
			{
				EngUICommon.showMessageBox(getShell(),AccLangKeys.MSG_PLEASE_ENTER_PARENT_ACCOUNT,SWT.ICON_INFORMATION);
				this.txtParentAccount.setFocus();
				return false;
			}
			else if (txtAccAccountCode.getText().trim().equals(""))
			{
				EngUICommon.showMessageBox(getShell(),AccLangKeys.MSG_PLEASE_FILL_ACCOUNT_CODE);
			...
			if (parentId.intValue() != -1)
			{
				...
				{
					EngUICommon.showMessageBox(getShell(),AccLangKeys.MSG_ACCOUNT_CODE_SHOULD_START_WITH_PARENT_ACCOUNT);
	...
	public String saveAccount()
	{
		try
		{
			...
			EngTXCommon.doTransactionTX("Acc_saveAccount", argMap);
