 ...
 * @version  $Id: AdmUIGroupPermissions.java,v 1.1 2007/01/22 17:02:40 huseyiner Exp $
 */
...
public class AdmUIGroupPermissions extends org.eclipse.swt.widgets.Composite implements SecureComposite, SearchComposite
{
	...
	private void initGUI()
	{
		try
		{
			...
			{
				tableGroupPermissions = new Table(this, SWT.FULL_SELECTION);
	...
	public void postInitGUI()
	{
		try
		{
			HashBag groupBag =(HashBag)EngTXCommon.doSelectTX("Adm_getGroups",null);
			...
			HashBag moduleBag =(HashBag)EngTXCommon.doSelectTX("Adm_getModules",null);
            ...
            HashBag userPermBag = (HashBag)EngTXCommon.doSelectTX("Adm_getUserPermissonLevels",null);
	...
	public void fillComboModuleComponents(int module_id)
	{
		try
		{
			...
			HashBag compListBag =(HashBag)EngTXCommon.doSelectTX("Adm_getModuleComponents",argMap);
	...
	public void fillTableUserPermissions()
	{
		try
		{
			...
			HashBag groupPermBag =(HashBag)EngTXCommon.doSelectTX("Adm_getGroupPermissions",null);
			TableItem item;
			String groupname;
			String module;
			String moduleComp;
	...
	public void save()
	{
		try
		{
			if (verifyFields())
			{
				...
				argMap.put(AdmKeys.ADM_GROUP_ID,comboGroups.getData(comboGroups.getText()));
				...
				EngTXCommon.doTransactionTX("Adm_saveGroupPermission",argMap);
				newForm();
				fillTableUserPermissions();
		...
		try
		{
			if (msg.open() == SWT.OK)
			{
				...
				if (items.length > 0)
				{
					...
					EngTXCommon.doTransactionTX("Adm_deleteGroupPermission",argMap);
					fillTableUserPermissions();
