...
public class AdmUICurrencyExchangeRateAdd extends org.eclipse.swt.widgets.Composite implements SecureComposite
{
	...
	private void initGUI()
	{
		try
		{
			...
			txtBaseCurrency.setForeground(SWTResourceManager.getColor(255, 255, 255));
			//END << txtBaseCurrency
	...
	public void fillCurrencyCombo()
	{
		try
		{
			...
			for (int k = 0; k < currencies.size(); k++)
			{
					...
					Boolean constant=(Boolean)currencyMap.get(EngKeys.CONSTANT);
					...
					{
						txtBaseCurrency.setText((String)currencyMap.get(EngKeys.CURRENCY_ABBR));
					}
				}
		}
		catch (Exception ex)
	...
	public void save()
	{
		...
		try
		{
			if (verifyFields())
			{
				...
				EngTXCommon.doTransactionTX("Adm_saveExchangeRate",argMap);
