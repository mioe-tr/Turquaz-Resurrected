 ...
 * @version  $Id: AdmUIUserPermissions.java,v 1.1 2007/01/22 17:02:40 huseyiner Exp $
 */
...
public class AdmUIUserPermissions extends org.eclipse.swt.widgets.Composite implements SecureComposite, SearchComposite
{
	...
	public void postInitGUI()
	{
		try
		{
			HashBag userBag =(HashBag)EngTXCommon.doSelectTX("Adm_getUsers",null);
	...
	public void fillComboModuleComponents(int module_id)
	{
		try
		{
			...
			HashBag moduleCompBag =(HashBag)EngTXCommon.doSelectTX("Adm_getModuleComponents",argMap);
            
            HashMap moduleCompMap = (HashMap) moduleCompBag.get(AdmKeys.ADM_MODULE_COMP);
            
			for (int i = 0; i < moduleCompMap.size(); i++)
			{
	...
	public void fillTableUserPermissions()
	{
		try
		{
			...
			HashBag userPermBag =(HashBag)EngTXCommon.doSelectTX("Adm_getUserPermissions",null);
	...
	public void save()
	{
		try
		{
			if (verifyFields())
			{
				...
				EngTXCommon.doSelectTX("Adm_saveUserPermission",argMap);
				newForm();
				fillTableUserPermissions();
		...
		try
		{
			if (msg.open() == SWT.OK)
			{
				...
				if (items.length > 0)
				{
					...
					EngTXCommon.doTransactionTX("Adm_deleteUserPermission",argMap);
					fillTableUserPermissions();
