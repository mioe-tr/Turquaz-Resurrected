...
/**
 * @author  Onsel
 * @version  $Id: CashUICashCardAbstract.java,v 1.1 2007/01/22 17:02:47 huseyiner Exp $
 */
...
public class CashUICashCardAbstract extends org.eclipse.swt.widgets.Composite implements SearchComposite
{
	...
	private void initGUI()
	{
		try
		{
			...
			this.setSize(631, 395);
			{
				...
				compSearchPanelLData.heightHint = 110;
				...
				}
				{
					...
					comboTypeLData.widthHint = 134;
					comboTypeLData.heightHint = 17;
					comboType = new CCombo(compSearchPanel, SWT.NONE);
	...
	public void fillComboType()
	{
		try
		{
			HashBag typeBag=(HashBag)EngTXCommon.doSelectTX("Csh_getCashTransactionTypes",null);
			
			HashMap types=(HashMap)typeBag.get(CashKeys.CASH_TRANS_TYPES);
			comboType.add("");
			for(int k=0; k<types.size(); k++)
			{
				HashMap type=(HashMap)types.get(new Integer(k));
				Integer typeId=(Integer)type.get(CashKeys.CASH_TRANS_TYPE_ID);
				String typeName=(String)type.get(CashKeys.CASH_TRANS_TYPE_NAME);
				
				comboType.add(typeName);
				comboType.setData(typeName,typeId);				
			}
		}
		catch(Exception ex)
		{
	...
	public void search()
	{
		try
		{
			if (verifyFields())
			{
				...
				argMap.put(CashKeys.CASH_CARD_ID,cashCardPicker.getId());
				argMap.put(EngKeys.DATE_START,datePicker.getDate());
				argMap.put(EngKeys.DATE_END,datePickerEndDate.getDate());
				argMap.put(CashKeys.CASH_DAILY, new Boolean(false));
				argMap.put(CashKeys.CASH_TRANS_TYPE_ID,comboType.getData(comboType.getText()));
				argMap.put(CashKeys.CASH_TRANS_TYPE_NAME,comboType.getText());
				
				HashBag abstractBag=(HashBag)EngTXCommon.doSelectTX("Csh_getCashCardAbstract",argMap);
				
				
				List deferred =(List)abstractBag.get(CashKeys.CASH_DEFERRED);
				List transList=(List)abstractBag.get(CashKeys.CASH_TRANSACTIONS);
	...
	private void tableCashTransMouseDoubleClick(MouseEvent evt)
	{
		try
		{
			...
			if (selection.length > 0)
			{
				...
				{
					updated = new CashUICashCollectTransactionUpdate(this.getShell(), SWT.NULL, cashBag).open();
				}
				else if (((Integer)cashBag.get(CashKeys.CASH_TRANS_TYPE_ID)).intValue() == EngBLCommon.CASH_CURRENT_PAYMENT)
				{
					updated = new CashUICashPaymentTransactionUpdate(this.getShell(), SWT.NULL, cashBag).open();
				}
				else if (((Integer)cashBag.get(CashKeys.CASH_TRANS_TYPE_ID)).intValue() == EngBLCommon.CASH_OTHER_COLLECT)
				{
					updated = new CashUICashOtherCollectTransactionUpdate(this.getShell(), SWT.NULL, cashBag).open();
				}
				else if (((Integer)cashBag.get(CashKeys.CASH_TRANS_TYPE_ID)).intValue() == EngBLCommon.CASH_OTHER_PAYMENT)
				{
					updated = new CashUICashOtherPaymentTransactionUpdate(this.getShell(), SWT.NULL, cashBag).open();
				}
				else if (((Integer)cashBag.get(CashKeys.CASH_TRANS_TYPE_ID)).intValue() == EngBLCommon.CASH_TRANSFER_BETWEEN_CARDS)
				{
					updated = new CashUICashTransferBetweenCardsUpdate(this.getShell(), SWT.NULL, cashBag).open();
				}
				if (updated)
					search();
			}
