...
/**
 * @author onsel
 * @version $Id: CashUICashCardSearch.java,v 1.1 2007/01/22 17:02:47 huseyiner Exp $
 */
public class CashUICashCardSearch extends org.eclipse.swt.widgets.Composite implements SearchComposite
{
		...
		try
		{
			...
			if (selection.length > 0)
			{
				...
				if (okToDelete)
				{
					...
					argMap.put(CashKeys.CASH_CARD_ID, row.getDBObject());
	...
	public void search()
	{
		try
		{
			...
			HashBag result  =(HashBag)EngTXCommon.doSelectTX("Csh_searchCashCard",argMap);			
	...
	public void treeMouseDoubleClick()
	{
		...
		if (selection.length > 0)
		{
			TableItem item = selection[0];
			ITableRow row = (ITableRow) item.getData();
			new CashUICashCardUpdate(this.getShell(), SWT.NULL, (Integer) row.getDBObject()).open();
			search();
		}
	}
}