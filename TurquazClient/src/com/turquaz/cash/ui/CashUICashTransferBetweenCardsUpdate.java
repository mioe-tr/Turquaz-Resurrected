...
/**
 * @author  Onsel
 * @version  $Id: CashUICashTransferBetweenCardsUpdate.java,v 1.1 2007/01/22 17:02:47 huseyiner Exp $
 */
...
* 
*/
public class CashUICashTransferBetweenCardsUpdate extends Dialog   implements SaveDialogInterface
{
	...
	public CashUICashTransferBetweenCardsUpdate(Shell parent, int style, HashBag cashBag)
	{
		super(parent, style);
	...
	public void postInitGUI()
	{
		try
		{
			...
			compTransAdd.getTxtCashCardWithCredit().setText(
					cashBag.get(CashKeys.CASH_CARD_WITH_CREDIT).toString());
			compTransAdd.getTxtCashCardWithDept().setText(
					cashBag.get(CashKeys.CASH_CARD_WITH_DEPT).toString());
			
			if (((BigDecimal) cashBag.get(CashKeys.CASH_TRANS_ROW_FOREIGN_DEPT_AMOUNT)).doubleValue()>0)
			{
	...
	public void save()
	{
		try
		{
			if (compTransAdd.verifyFields())
			{
				...
				EngTXCommon.doTransactionTX("Csh_updateTransBetweenCards",argMap);
	...
	return	CashUICashTransferBetweenCards.class.getName();
